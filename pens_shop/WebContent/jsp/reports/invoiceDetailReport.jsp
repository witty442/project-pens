...
function gotoReport(path){
	if(    $('#orderDateFrom').val()=='' && $('#orderDateTo').val()==''
       ...
       if( $('#orderDateFrom').val()!= '' || $('#orderDateTo').val()!= ''){
    	   if( $('#orderDateFrom').val()== ''){
    		   alert('กรุณาใส่วันที่เริ่มต้นให้ครบ');
    		   $('#orderDateFrom').focus();
    		   return;
    	   }
    	   if( $('#orderDateTo').val()== ''){
    		   alert('กรุณาใส่วันที่สิ้นสุดให้ครบ');
    		   $('#orderDateTo').focus();
    		   return;
    	   }
       }
       ...
       if( $('#orderNoFrom').val()!= '' || $('#orderNoTo').val()!= ''){
    	   if( $('#orderNoFrom').val()== ''){
    		   alert('กรุณาใส่เลขที่บิลเริ่มต้นให้ครบ');
    		   $('#orderNoFrom').focus();
    		   return;
    	   }
    	   if( $('#orderNoTo').val()== ''){
    		   alert('กรุณาใส่เลขที่บิลสิ้นสุดให้ครบ');
    		   $('#orderNoTo').focus();
    		   return;
    	   }
       }
	...
	document.invoiceDetailReportForm.action = path + "/jsp/invoiceDetailReportAction.do?do=searchReport&rf=Y";
	document.invoiceDetailReportForm.submit();
	return true;
}

function clearForm(path){
	document.invoiceDetailReportForm.action = path + "/jsp/invoiceDetailReportAction.do?do=clearForm";
	document.invoiceDetailReportForm.submit();
	return true;
}
</script>
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
	    	...
	    	<!-- PROGRAM HEADER -->
	      	<jsp:include page="../program.jsp">
				<jsp:param name="function" value="InvoiceDetailReport"/>
			</jsp:include>
	      	<!-- TABLE BODY -->
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		...
	      		<tr>
		            ...
		            <td bgcolor="#f8f8f8">
						<!-- BODY -->
						<html:form action="/jsp/invoiceDetailReportAction">
						...
						<table align="center" border="0" cellpadding="3" cellspacing="0" width="100%">
							...
							<tr>
								<td align="right">วันที่เริ่มต้่น :<html:text property="invoiceDetailReport.orderDateFrom" styleId="orderDateFrom" readonly="true" size="15"/></td>
								<td align="left">
								     วันที่สิ้นสุด :<html:text property="invoiceDetailReport.orderDateTo" styleId="orderDateTo" readonly="true" size="15"/>
								</td>
							</tr>
							<tr>
								<td align="right">เลขที่บิลขายเริ่มต้น: <html:text property="invoiceDetailReport.orderNoFrom" styleId="orderNoFrom"  size="15"/></td>
								<td align="left">
								เลขที่บิลขายสิ้นสุด : 
								     <html:text property="invoiceDetailReport.orderNoTo" styleId="orderNoTo"  size="15"/>
								</td>
							</tr>
							<tr>
								<td align="right">ตั้งแต่รหัสสินค้า : <html:text property="invoiceDetailReport.productCodeFrom" styleId="productCodeFrom"  size="15"/></td>
								<td align="left">
								ถึงรหัสสินค้า : 
								     <html:text property="invoiceDetailReport.productCodeTo" styleId="productCodeTo"  size="15"/>
								</td>
							</tr>
							<tr>
