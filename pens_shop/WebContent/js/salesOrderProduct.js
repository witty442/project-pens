...
function addProduct(path,objValue){
	...
	if(index < 0){
		...
		if(objValue.taxable =='Y'){
		  tbl.rows[index+1].cells[9].innerHTML ="<img border=0 src='"+path+"/icons/check.gif'/>";
		}else{
		  tbl.rows[index+1].cells[9].innerHTML = "";	
		}
		tbl.rows[index+1].cells[10].innerHTML = "";//promotion
...
function addProduct2(path,objValue){
    ...
    jQtable.each(function(){
        ...
        tds += '<td align="center"></td>';//1
        tds += '<td align="center"></td>';//2
        tds += '<td align="left"></td>';//3
        tds += '<td align="center"></td>';//4
        tds += '<td align="right"></td>';//5
        tds += '<td align="right"></td>';//6
        tds += '<td align="right"></td>';//7
        tds += '<td align="right"></td>';//8
        tds += '<td align="right"></td>';//9
        tds += '<td align="center"></td>';//10
        tds += '<td align="center"></td>';//11
        tds += '</tr>';
...
function setValueToProduct(path, objValue){
	...
	tbl.rows[objValue.row].cells[c++].innerHTML= "";//isPromotion
...
function createProductList(){
	...
	for(var i=0;i<ids.length;i++){
		inputLabel="";
		//alert("isPromotionSpecial:"+isPromotionSpecial[i].value);
...
function calculatePrice(){
	...
	for(var i=0;i<totals1.length;i++){
		...
		if(taxables[i].value=='Y'){
			...
			tbl.rows[i+1].cells[7].innerHTML=addCommas(Number(Number(afdiscs1[i].value) + Number(afdiscs2[i].value)).toFixed(2));
		}else{
