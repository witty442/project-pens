...
alter table t_order modify address_desc varchar(255);

alter table t_order_line add selling_price decimal(15,5);

alter table t_receipt_by add credit_card_no varchar(30);
...
alter table c_reference modify value varchar(255);
...
update  pens_shop.c_reference set value = '       ศุนย์การค้าเมญ่า เชียงใหม่ ห้อง 210/2 ชั้นที่ 2  เลขที่ 55, 55/5 หมู่ที่ 5       ถ.ซุปเปอร์ไฮเวย์ ต.ช้างเผือก อ.เมืองเชียงใหม่ เชียงใหม่ 50300 สาขาที่ 00005' where code = 'ADDRESS_MAYA';

alter table t_receipt_by modify creditcard_expired char(10);
update c_reference set reference_id = 1100 where reference_id = 1104 and code ='PaymentMethod';

alter table t_order add cancel_reason varchar(200);
