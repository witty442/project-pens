...
function search(path, type) {
	if(validateCriteria()){
	   document.aReportForm.action = path + "/jsp/aReportAction.do?do=search&rf=N&";
	   document.aReportForm.submit();
	   return true;
	}
	return false;
}
...
function searchOrder(path, type, field) {
       var search_url2= path + "/jsp/aReportAction.do?do=search&rf=N&order_type="+type+"&order_by_name="+field;
       //alert(search_url2);
	   document.aReportForm.action = search_url2;
	   document.aReportForm.submit();
	   return true;
}
...
function exportData(path,type){
	...
	document.aReportForm.action = path + "/jsp/aReportAction.do?do=export&columnCount="+columnCount+param;
	document.aReportForm.submit();
...
function getSQL(path, type) {
	if(validateCriteria()){
	   document.aReportForm.action = path + "/jsp/aReportAction.do?do=getSQL";
	   document.aReportForm.submit();
	   return true;
	}
	return false;
...
function saveProfile(path, type) {
	...
	if(profileId != '0'){
	   ...
	   document.aReportForm.action = path + "/jsp/aReportAction.do?do=saveProfile&reportName="+reportName;
	   document.aReportForm.submit();
	   return true;
	}
	return false;
...
function editProfile(path, type) {
	var profileId = $('#profileId').val();
	var reportName = document.getElementById("reportName").value; 
    var url = path + "/jsp/manageProfileSearchAction.do?do=prepare&action=new&profileId="+profileId+"&reportName="+reportName;
...
function changeProfile(path, profileId) {
	if(profileId != '0'){
		var reportName = document.getElementById("reportName").value; 
	   document.aReportForm.action = path + "/jsp/aReportAction.do?do=changeProfile&reportName="+reportName;
	   document.aReportForm.submit();
	   return true;
	}
	return false;
...
function clearForm(path, type) {
	document.aReportForm.action = path + "/jsp/aReportAction.do?do=prepare&action=new&type="+type;
	document.aReportForm.submit();
	return true;
}
...
function showSearchValuePopup(path,currCondNo){
	...
	if(currCondTypeValue =='-1'){
       alert("โปรดเลือก ประเภทขอบเขตข้อมูล");
		...
		param +="&decode=true&load=1&reportName="+reportName;
		//alert(param);
		...
		PopupCenter(encodeURI(url),"Search Popup",width-250,height);
	}
}
