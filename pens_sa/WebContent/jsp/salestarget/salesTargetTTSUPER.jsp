...
function validateTotalCanAccept(){
   ...
   if(total_ctn_all_by_sales != total_ctn_all_by_mkt ){
	   ...
	   document.getElementById('total_ctn_all_by_sales').className='errorBoldNumber';
	   document.getElementById('total_price_all_by_sales').className='errorBoldNumber';
	   
	   document.getElementById('total_ctn_all_by_mkt').className='errorBoldNumber';
	   document.getElementById('total_price_all_by_mkt').className='errorBoldNumber';
	   return false;
    }
    return true;
}
...
function validateTotalByCur(curCol,curRow){
	 //validate totalAll Sales vs MKT
   var total_ctn_all_by_sales = convetTxtObjToFloat(document.getElementById('total_ctn_all_by_sales'));
   var total_ctn_all_by_mkt = convetTxtObjToFloat(document.getElementById('total_ctn_all_by_mkt'));
   ...
   if(total_ctn_all_by_sales > total_ctn_all_by_mkt){
	   ...
	   //target_qty_objx.className='errorBoldNumber';
	   
	    /* var target_amount_objx =document.getElementById('target_amount_'+curCol+"_"+curRow);
	   var target_qty_tempx = convetTxtObjToFloat(target_qty_objx);
	   var target_amount_tempx = convetTxtObjToFloat(target_amount_objx);
	  
	   alert('target_qty_'+curCol+"_"+curRow+":total_ctn_all_by_sales:"+total_ctn_all_by_sales);
		  
	   //reset input cur
	   target_qty_objx.value ="";
	   target_amount_objx.value = ""; */
   }
}
...
function sumTotalByRow(obj,col,row){
   ...
   for(var colX=0;colX< maxColumns;colX++){
	   target_qty_obj =document.getElementById('target_qty_'+colX+"_"+row);
	   target_amount_obj =document.getElementById('target_amount_'+colX+"_"+row);
	   price_obj = document.getElementById('price_'+colX+"_"+row);
...
//SummAllRow
function sumTotalByAllCol(obj,curCol,curRow){
   var target_qty_bj ="";
   var target_amount_bbj ="";
   var price_obj ="";
   var target_qty_temp = 0;
   ...
   //Validate total All
   validateTotalByCur(curCol, curRow);
}
									...
									%>
									     <td class="td_number" title="<%=salesrep.getSalesrepCode()%>">									        
									        <input type="text" size="5"  class="enableNumber" autoComplete="off"
									           ...
									           onblur="isNum(this);" 
									           onchange="sumTotalByRow(this,'<%=col%>','<%=row%>')"
									           value="<%=targetQty%>"  title="<%=salesrep.getSalesrepCode()%>"/>
