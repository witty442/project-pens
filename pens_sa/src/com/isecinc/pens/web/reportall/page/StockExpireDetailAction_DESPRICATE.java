...
/**
 *Despricate Case :Duplicate
 * 
 * @author WITTY
 * 
 */
public class StockExpireDetailAction_DESPRICATE extends I_Action {
	...
	public String prepare(ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if("new".equals(action)){
				...
				stockBean.setCompareDate("compareStockDate");
				stockBean.setBrandGroupType("NISSIN");
	...
	public String search(ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if("newsearch".equalsIgnoreCase(action) || "back".equalsIgnoreCase(action)){
				...
				//get Total Record
				aForm.setTotalRecord(StockExpireDetailDAO_DESCRICATE.searchStockReturnListTotalRec(conn,aForm.getBean().getStockBean(),user));
				//calc TotalPage
				...
				StockBean stockResult = StockExpireDetailDAO_DESCRICATE.searchStockReturnList(conn,user,aForm,allRec,currPage,pageSize,excel);
	...
	public ActionForward export(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			reportAllForm.getBean().getStockBean().setDataStrBuffer(null);
			reportAllForm.getBean().getStockBean().setItemsList(null);
			
			conn = DBConnection.getInstance().getConnectionApps();
			
			StockBean stockBean = StockExpireDetailDAO_DESCRICATE.searchStockReturnList(conn, user, reportAllForm, true, 0, pageSize, true);
			if(stockBean != null && stockBean.getDataStrBuffer() != null){
				java.io.OutputStream out = response.getOutputStream();
				response.setHeader("Content-Disposition", "attachment; filename=data.xls");
				response.setContentType("application/vnd.ms-excel");
				
				Writer w = new BufferedWriter(new OutputStreamWriter(out,"UTF-8")); 
				w.write(stockBean.getDataStrBuffer().toString());
			    w.flush();
			    w.close();
