...
public class AGenerate {
	...
	public AGenerate(AInitial aaInit){
		aInit = aaInit;
	}
	/**
	 * Gen Report 
 ...
 * @throws Exception
 */
   public  StringBuffer genHeaderReportExportExcel(String reportName,User user,ABean salesBean,String columnCount,String condDisp1,String condDisp2,String condDisp3,String condDisp4,String condDisp5) throws Exception{
		
		StringBuffer htmlStr = new StringBuffer("");
		try{
			htmlStr.append("<table border='1' width='100%' cellpadding='3' cellspacing='1' class='result'> \n");
			htmlStr.append("<tr> \n");
			logger.debug("XX reportName:"+reportName);
			
			if(reportName.equalsIgnoreCase("SalesAnalyst")){
			   
			   htmlStr.append(" <td colspan='"+columnCount+"'><b>Sales Analysis Report</b></td>  \n");
			}else if(reportName.equalsIgnoreCase("ProjectCAnalyst")){
			   htmlStr.append(" <td colspan='"+columnCount+"'><b>ProjectC Analysis Report</b></td>  \n");
			}
			htmlStr.append("</tr> \n");
			htmlStr.append("<tr> \n");
    ...
    /*private static String[][] genDataToArray(String contextPath,ConfigBean groupByBean,List colGroupList,List colDispList ,ResultSet rs,String summaryType) throws Exception{
	  String[][]
    	try{
				...
				for(gc=0;gc<colGroupList.size();gc++){
					...
					for(col=0;col<colDispList.size();col++){
						...
						if(resultKey.startsWith("CALL") && !resultKey.startsWith("CALL_NEW") && isSummry && AConstants.SUMMARY_TYPE_SUM.equals(summaryType)){
							...
							/** Sum No Dup **/
							//value = Utils.isNullToZero(rs.getBigDecimal(AInitial.NO_DUP_PREFIX+resultKey));//ND_Normal
