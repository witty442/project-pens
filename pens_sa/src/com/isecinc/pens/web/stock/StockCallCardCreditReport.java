...
public class StockCallCardCreditReport {
	...
	public static StockBean searchReport(String screenWidth,String contextPath ,StockBean o,User user,boolean excel){
	    //logger.debug("excel:"+excel);
	    return searchReportModel(screenWidth,contextPath,o,user,excel);
	}
	
	public static StockBean searchReportModel(String screenWidth,String contextPath,StockBean o,User user,boolean excel){
		StockBean item = null;
		...
		try{
			...
			sql.append("\n  ,sum(pri_qty) as qty,sum(sec_qty) as sec_qty ");
			...
			sql.append("\n ) M WHERE M.qty <> 0 OR M.sec_qty <> 0 ");
			sql.append("\n UNION ALL ");
			...
			sql.append("\n , SUM((INVOICED_QTY+Promotion_QTY)) as qty ,0 as sec_qty");
			...
			sql.append("\n AND M.INVOICE_DATE >= to_date('"+startDateStr+"','dd/mm/yyyy')");//start init date check
			sql.append("\n AND ( ( M.INVOICED_QTY <> 0 ) OR  (( M.Promotion_QTY <> 0) AND M.Promotion_QTY >0) )");
			//debug 
			...
			while (rst.next()) {
			  ...
			  item.setSecQty(Utils.decimalFormat(rst.getDouble("sec_qty"), Utils.format_current_2_disgit));
			...
			if(dataMAP != null && !dataMAP.isEmpty()){
			   ...
			   Map<String, String> sumRequestDateToMap = sumRequestDateToMap(screenWidth,requestDateList, dataMAP);
			   		   
			   //Gen Header Table
			   html.append(genHeadTable(screenWidth,contextPath, o, excel, productList,requestDateList,sumRequestDateToMap));
	...
	private static Map<String, String> sumRequestDateToMap(String screenWidth,List<StockBean> requestDateList, Map<String, StockBean> dataMAP) throws Exception{
		...
		for(int i=0;i<requestDateList.size();i++){
			...
			while(itsAll.hasNext()){
				...
				if(key.startsWith("S_"+requestDate)){
				   ...
				   //Case Stock check sec_qty is not 0
				   sumRequestDateAll += Utils.convertStrToDouble(valueBean.getSecQty());
				}
			}//while
	 ...
	 * @throws Exception
	 */
	private static StringBuffer genHeadTable(String screenWidth, String contextPath,StockBean head,boolean excel
...
*/
		h.append("<thead> \n");
		h.append("<tr> \n");
		h.append(" <th width='10%' class='td_bg_lineH'>");
		...
		if(head.getReportType().equalsIgnoreCase("2")){
			for(int i=0;i<requestDateList.size();i++){
				...
				if( head.getDispOrderOnly().equals("")){ //No check display order only
					...
					if( Utils.convertStrToDouble(sumRequestDateToMap.get(item.getRequestDate()+"_S")) != 0){
						colSpan++;
					   h.append("<th width='"+tdWidth+"%' class='td_bg_lineS'>"+requestDateTemp+"</th> \n");
					}
				}//if
				...
				if(Utils.convertStrToDouble(sumRequestDateToMap.get(item.getRequestDate()+"_A")) != 0){
					colSpan++;
				   h.append("<th width='"+tdWidth+"%' class='td_bg_lineA'>"+requestDateTemp+"</th> \n");
				}
			}
		}else{
			String productName = "";
			for(int i=0;i<productList.size();i++){
				...
				h.append("<th width='"+tdWidth+"%' class='td_bg_lineH'>"+item.getItemCode()+"<br/> "+productName+"</th> \n");
			}
		}
		h.append("</tr> \n");
		h.append("</thead> \n");
	...
	private static StringBuffer genRowTable(String contextPath,StockBean head,boolean excel
		...
		if(head.getReportType().equalsIgnoreCase("2")){ //แสดง SKU แนวนอน
		  for(int i=0;i<productList.size();i++){
			...
			h.append("<tr> \n");
			h.append(" <th class=''>"+item.getItemCode()+"-"+item.getItemName()+"</th> \n");
			for(int n=0;n<requestDateList.size();n++){
				...
				if( head.getDispOrderOnly().equals("") && Utils.convertStrToDouble(sumRequestDateToMap.get(itemS.getRequestDate()+"_S")) != 0){
					...
					logger.debug("DEBUG key["+key+"]itemResult["+itemResult+"]");
					
					if(itemResult != null){
						if( !Utils.isNull(itemResult.getOrderQty()).equals("")){
					       h.append(" <td class='td_bg_lineS_num'>"+linkPre+Utils.isNull(itemResult.getOrderQty())+linkPost+"</td> \n");
						}else{
						   h.append(" <td class='td_bg_lineS_num'>"+linkPre+"0.00"+linkPost+"</td> \n");
						}
					}else{
						h.append(" <td class='td_bg_lineS_num'>"+linkPre+""+linkPost+"</td> \n");
					}
				}//if
				...
				if( Utils.convertStrToDouble(sumRequestDateToMap.get(itemS.getRequestDate()+"_A")) != 0){
					...
					if(itemResult != null){
					   ...
					   h.append(" <td class='td_bg_lineA_num'>"+linkPre+""+linkPost+"</td> \n");
					}
				}//if
			}//for 2
			h.append("</tr> \n");
		 ...
		 //////////////////////////////////////////////////
			  ...
			  for(int i=0;i<requestDateList.size();i++){
				...
				if( head.getDispOrderOnly().equals("") && Utils.convertStrToDouble(sumRequestDateToMap.get(item.getRequestDate()+"_S")) != 0){
					...
					h.append(" <th class='td_bg_lineS'>"+requestDateTemp+"</th> \n");
				...
				if(Utils.convertStrToDouble(sumRequestDateToMap.get(item.getRequestDate()+"_A")) != 0){
					...
					//convert to normal date
					requestDateTemp = item.getRequestDate().substring(6,8)+"/"+item.getRequestDate().substring(4,6)+"/"+item.getRequestDate().substring(0,4); 
					h.append(" <th class='td_bg_lineA'>"+requestDateTemp+"</th> \n");
					for(int n=0;n<productList.size();n++){
						itemS = productList.get(n);
