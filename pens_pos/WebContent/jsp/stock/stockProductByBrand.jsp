...
try{	
	if(categoryCode != null && categoryCode.length()>0){
		catalogs = new MProduct().getStockProductCatalogByBrand(categoryCode,orderDate,priceListId,user);
	}
}catch(Exception e){
	e.printStackTrace();
}
...
<td align="center"><input name="qty1" type="text" class="qtyInput" onkeydown="return inputNum2(event,this);" onblur="isNumeric(this);linePrice('<%=rowNo%>','<%=catalog.getPrice1()%>','<%=catalog.getPrice2()%>')" value="<%=qty1%>" />
<!-- &nbsp;&frasl;&nbsp; -->
	<input name="qty2" type="hidden" class="qtyInput" <%=disable%> onkeydown="return inputNum2(event,this);" onblur="isNumeric(this);linePrice('<%=rowNo%>','<%=catalog.getPrice1()%>','<%=catalog.getPrice2()%>')" value="<%=qty2%>" />

    <input name="totalLineAmtT" type="hidden" value="<%=lineAmt%>" />
	...
	<input name="expireDate" type="hidden"  />
</td>
</tr>
<% 
rowNo++;
