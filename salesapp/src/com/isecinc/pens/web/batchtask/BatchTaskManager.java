...
public BigDecimal runBatchFromManualNoWait(User userLogin,String pageName,BatchTaskForm batchTaskForm,Map<String,String> batchParamMap)throws Exception {
		logger.debug("runBatchFromManualNoWait");
		BatchTaskDAO dao = new BatchTaskDAO();
		boolean canRunBatch = false;
		EnvProperties env = EnvProperties.getInstance();
		BigDecimal transactionID = new BigDecimal("0");
		try {
			/** insert to monitor_interface **/
			MonitorBean monitorModel = new MonitorBean();
			monitorModel.setName(pageName);
			...
			//create Batch Task and RunBatchTaskWorker
			MonitorBean m = createBatchTask(monitorModel,userLogin);
	...
	public MonitorBean createBatchTask(MonitorBean monitorModel,User user) throws Exception{
		Connection connMonitor = null;
		BatchTaskDAO dao = new BatchTaskDAO();
		try{
			connMonitor = DBConnection.getInstance().getConnection();
