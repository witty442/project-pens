...
public class BatchTaskDAO {
	...
	public MonitorBean insertMonitor(Connection conn ,MonitorBean model) throws Exception {
		...
		try {
			String sql = "INSERT INTO PENSONLINE.monitor(" +
			" transaction_id," +
			" monitor_id," +
			" name ," +
			" type ," +
	...
	public MonitorItemBean insertMonitorItem(Connection conn,MonitorItemBean model) throws Exception {
		...
		PreparedStatement ps = null;
		try {
			String sql = "INSERT INTO PENSONLINE.monitor_item(" +
			" id,"+
			" monitor_id ,"+
			" source ," +
			" destination ," +
	...
	public MonitorItemBean insertMonitorItemDetail(Connection conn ,MonitorItemDetailBean[] monitorItemDetailBean ) throws Exception {
		...
		try {
			...
			" type ," +
			" amount )" +
			" VALUES ((select max(id) from PENSONLINE.monitor_item),?,?,?,?,?) ";
	...
	public  BigDecimal findMaxMonitorId(Connection conn,MonitorBean model) throws Exception{
		...
		try{
			...
			sql.append(" select max(monitor_id) as monitor_id from PENSONLINE.monitor \n");
	...
	public  String findMonitorStatus(Connection conn,String id) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select channel as batch_task_status from PENSONLINE.monitor \n");
			sql.append("where MONITOR_ID = (select min(monitor_id) from PENSONLINE.monitor where transaction_id ="+id+") \n");
    ...
    public static void clearTaskControlMonitorAll() {
    	...
    	try {
            conn = DBConnection.getInstance().getConnection();
	       	String sql = "UPDATE PENSONLINE.c_monitor SET  transaction_id = 0";
			ps = conn.prepareStatement(sql);
            ps.executeUpdate();
    ...
    public void updateControlMonitor(BigDecimal transactionId,String type) {
    	...
    	try {
           conn = DBConnection.getInstance().getConnection();
           updateControlMonitor(conn,transactionId,type);
        } catch (Exception e) {
        	e.printStackTrace();
        }finally{
        	try{
    ...
    public void updateControlMonitor(Connection conn,BigDecimal transactionId,String action) throws Exception {
		PreparedStatement ps = null;
		try {
			String sql = "UPDATE PENSONLINE.c_monitor SET  transaction_id = "+transactionId+" WHERE action = '"+action+"'";
			...
			if(r==0){
				//insert 
				sql = "insert into PENSONLINE.c_monitor(action,transaction_id)values('"+action+"',"+transactionId+")";
	...
	public  boolean canRunBatchTask(String action) throws Exception{
		...
		try{
			...
			sql.append(" select name,IS_CONCURRENT from pensonline.BATCH_TASK_CONTROL where name ='"+action+"' \n");
			...
			if( !isConcurrent){
				...
				sql.append(" select * from PENSONLINE.c_monitor where action ='"+action+"' \n");
	...
	public void updateControlStatusMonitor(Connection conn,BigDecimal transactionId,BigDecimal monitorId,String status) throws Exception {
		PreparedStatement ps = null;
		try {
			String sql = "UPDATE PENSONLINE.monitor SET  channel = ? WHERE MONITOR_ID = ? and transaction_id = ?";
	...
	public  MonitorBean findMonitorBean(BigDecimal monitorId) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select * from PENSONLINE.monitor where 1=1  and monitor_id ="+monitorId+"\n");
	...
	public  MonitorBean[] findMonitorListNew(User user,String name) throws Exception{
		...
		try{
			...
			sql.append(" , (select count(*) from PENSONLINE.monitor_item where monitor_item.monitor_id = monitor.monitor_id and status = "+BatchTaskConstants.STATUS_SUCCESS+" )  as success_count  \n");
			sql.append(" , (select max(id) from PENSONLINE.monitor_item where monitor_item.monitor_id = monitor.monitor_id)  as monitor_item_id  \n");
			sql.append(" , (select error_msg from PENSONLINE.monitor_error_mapping where monitor_error_mapping.error_code = monitor.error_code) as error_disp \n");
			sql.append(" from PENSONLINE.monitor  \n");
			...
			while(rs.next()){
				...
				m.setCreateDate(rs.getDate("create_date"));
				m.setSubmitDate(rs.getTimestamp("submit_date"));
				m.setSubmitDateDisp(DateUtil.stringValue(rs.getTimestamp("submit_date"),DateUtil.DD_MM_YYYY__HH_mm_ss_WITH_SLASH,DateUtil.local_th));
	            m.setFileCount(rs.getInt("file_count"));
	            m.setSuccessCount(rs.getInt("success_count"));
	            if( !Utils.isNull(rs.getString("error_msg")).equals("")){
	               m.setErrorMsg(Utils.isNull(rs.getString("error_msg")));
	            }else{
	...
	public  MonitorBean[] findMonitorList(User user,String type) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select monitor.*  \n");
			sql.append(" , (select count(*) from monitor_item where monitor_item.monitor_id = monitor.monitor_id and status = "+BatchTaskConstants.STATUS_SUCCESS+" )  as success_count  \n");
	...
	public  MonitorBean[] findMonitorDetailList(User user ,MonitorBean mc,String orderBy) throws Exception{
		...
		try{
            ...
            if( mc.getStatus() != 0){
            	if(mc.getStatus() == BatchTaskConstants.STATUS_SUCCESS){
	...
	public  MonitorItemBean findMonitorItemBean(User user, MonitorBean mc) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select *   \n");
			sql.append(" from PENSONLINE.monitor_item  \n");
			sql.append(" where 1=1 \n");
	...
	public  MonitorItemBean findMonitorItemBeanByPK(User user, String id) throws Exception{
		...
		try{
			...
			sql.append(" from PENSONLINE.monitor_item where 1=1 \n");
	...
	public  List<MonitorItemBean> findMonitorItemList(User user, MonitorBean mc,String typeImport) throws Exception{
		...
		try{
			...
			while(rs.next()){
				...
				/*if(typeImport.equalsIgnoreCase(BatchTaskConstants.TYPE_IMPORT_WACOAL_SALESIN_RETURN)){
				   //SalesInList
	...
	public  List<MonitorItemResultBean> findMonitorItemResultList(Connection conn ,BigDecimal monitorIItemId,String statusDesc) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select * from PENSONLINE.monitor_item_result where 1=1 \n");
	...
	public String findMonitorItemColumnHeadResultList(Connection conn ,BigDecimal monitorIItemId) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select * from PENSONLINE.MONITOR_ITEM_COLUMN_RESULT where 1=1 \n");
			sql.append(" and monitor_item_id ="+monitorIItemId +"\n");
		    logger.debug("SQL:"+sql.toString());
	...
	public  MonitorItemDetailBean[] findMonitorItemDetailBeanList(BigDecimal monitorItemId) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select * from PENSONLINE.monitor_item_detail where 1=1  and monitor_item_id ="+monitorItemId+"\n");
	...
	public MonitorBean updateMonitor(Connection conn,MonitorBean model) throws Exception {
		...
		try {
			String sql = "UPDATE PENSONLINE.monitor SET " +
	...
	public MonitorItemBean updateMonitorItem(Connection conn,MonitorItemBean model) throws Exception {
		PreparedStatement ps = null;
		try {
			String sql = "UPDATE PENSONLINE.monitor_item SET " +
	...
	public MonitorBean updateStatusReGenToMonitorAndItem(Connection conn,MonitorBean model) throws Exception {
		...
		try {
			String sql = "UPDATE PENSONLINE.monitor SET  status = "+BatchTaskConstants.STATUS_REGEN+"  WHERE  transaction_id ="+model.getTransactionId();
			String sql2 = "UPDATE PENSONLINE.monitor_item SET  status = "+BatchTaskConstants.STATUS_REGEN+"  WHERE monitor_id in(select monitor_id from monitor where  transaction_id ="+model.getTransactionId()+")";
 ...
 public  static String getStatDesc(int status){
		String statusDesc ="";
		if(BatchTaskConstants.STATUS_START==status){
			statusDesc = "Start";
		}else 	if(BatchTaskConstants.STATUS_SUCCESS==status){
			statusDesc = "SUCCESS";
		}else 	if(BatchTaskConstants.STATUS_FAIL==status){
			statusDesc = "FAIL";
		}
		return statusDesc;
	}
