...
public class FTPManager {
				...
				for (i = 0; i < ftpFiles.length; i++) {
					...
					for (int j = 1; it.hasNext(); j++) {
						...
						if (canGetFtpFile == true) {
							...
							/** 2 **/
							String dataStreamStr = convertStreamToString(ftp.retrieveFileStream(ftpFileName), tableName,
									userBean);
	...
	public String getDownloadFTPFileByName(String pathFullName) throws Exception {
		...
		try {
			...
			logger.debug("Get Stream FTP Response " + ftp.getControlEncoding() + " :" + ftp.getReplyCode() + "-"
					+ ftp.getReplyString());
	...
	public String[] getManualScriptImportExport(String prefix, String salesType, String salesCode, String encoding)
			throws Exception {
		FTPClient ftp = null;
	...
	public String getDownloadFTPFileByName(String pathFullName, HttpServletResponse response) throws Exception {
		...
		try {
			...
			logger.debug("pathGetFileFull:" + pathFullName);
			// response.setHeader("Content-Disposition", "attachment; filename=logs." +
			// "csv");
	 ...
	 * @throws Exception
	 */
	public void uploadFileToFTP(String path, LinkedHashMap<String, TableBean> controlTableMap, User userBean)
			throws Exception {
		String ftpResponse = "";
		FTPClient ftp = null;
		try {
				...
				if (tableBean.getDataStrBuffList() != null && tableBean.getDataStrBuffList().size() > 0) {
					...
					for (int k = 0; k < tableBean.getDataStrBuffList().size(); k++) {
						...
						if (dataStrBuff != null) {
							...
							OutputStream out = ftp
									.storeFileStream(fileBean.getFileName() + "-" + userBean.getUserName() + ".log");
	 ...
	 * 
	 * @param controlTableMap
	 * @throws Exception ExportManager Call
	 */
	/** Process Work on Window or (some tablet V107) **/
	@Deprecated
	public void uploadAllFileToFTP_ORG1(LinkedHashMap<String, TableBean> controlTableMap, String path)
			throws Exception {
		FTPClient ftp = null;
		String reply = "";
		try {
				...
				if (tableBean.getFileExportList() != null && tableBean.getFileExportList().size() > 0) {
					...
					for (int f = 0; f < tableBean.getFileExportList().size(); f++) {
						...
						Writer w = new BufferedWriter(
								new OutputStreamWriter(out, Constants.FTP_EXPORT_TO_ORACLE_ENCODING_TIS_620));
						...
						logger.info(" End completePendingCommand completed[" + completed + "]FTP Response "
								+ ftp.getControlEncoding() + " :" + ftp.getReplyString());
						...
						logger.info("End CHMOD 777 FTP sendSiteCommand Response " + ftp.getControlEncoding() + " :"
								+ ftp.getReplyString());

					} // for

				} else if (tableBean.getDataStrExport() != null
					...
					logger.info(
							"Write fileName FTP Response " + ftp.getControlEncoding() + " :" + ftp.getReplyString());
	...
	// TEST METHOD
	public void uploadAllFileToFTP_OPT2_TEST(LinkedHashMap<String, TableBean> controlTableMap, String path)
			throws Exception {
		try {
			Set s = controlTableMap.keySet();
			Iterator it = s.iterator();
	 ...
	 * @throws Exception
	 */
	public void uploadAllFileToFTP(User userBean, LinkedHashMap<String, TableBean> controlTableMap, String path)
			throws Exception {
		try {
			Set s = controlTableMap.keySet();
			Iterator it = s.iterator();
	...
	private void uploadAllFileToFTP_BY_FILE(String path, TableBean tableBean) throws Exception {
		...
		try {
			...
			if (tableBean.getDataStrExport() != null
				...
				/*
				 * //Test Write File Local String fileTemp =
				 * "D:/temp/"+tableBean.getFileFtpNameFull();
				 * logger.info("Write File To Temp Test:"+fileTemp);
				 * FileUtil.writeFile(fileTemp, tableBean.getDataStrExport().toString());
				 */
				/******************************************************************/
	...
	private void uploadImageAllFileToFTP(User user, TableBean tableBean) throws Exception {
		...
		try {
			...
			// overWrite =
			// EnvProperties.getInstance().getProperty("path.master.customer.images.overwrite");
			...
			if (tableBean.getImageFileList() != null && tableBean.getImageFileList().size() > 0) {
				...
				for (int i = 0; i < tableBean.getImageFileList().size(); i++) {
					...
					if (done) {
						inputStream.close();
						logger.info("Upload Image File FTP Response " + ftp.getControlEncoding() + " :"
								+ ftp.getReplyString());
					...
					/** Close Command FTP **/
					/*
					 * try{ logger.info(" Start completePendingCommand "); boolean completed =
					 * ftp.completePendingCommand(); String ftpResponse = ftp.getReplyString();
					 * logger.info("ftpResponse["+ftpResponse+"]");
					 * logger.info(" End completePendingCommand completed["
					 * +completed+"]FTP Response "+ftp.getControlEncoding()+" :"+ftp.getReplyString(
		...
		try {
			...
			if (imageFileBean != null) {
				...
				if (done) {
					inputStream.close();
					logger.info(
							"Upload Image File FTP Response " + ftp.getControlEncoding() + " :" + ftp.getReplyString());
					logger.info("The is uploaded successfully.");
				}
			} else {
				// Data not Found
	...
	private void uploadCustomerLocationAllFileToFTP_OPT2_BY_FILE(User userBean, TableBean tableBean,
			StringBuffer dataExportSt) throws Exception {
		FTPClient ftp = null;
		String reply = "";
		Writer w = null;
		OutputStream out = null;
		logger.info("uploadCustomerLocationAllFileToFTP_OPT2_BY_FILE");
		String path = "";
		String fileNameFull = "";
		try {
			ftp = new FTPClient();
			...
			path = EnvProperties.getInstance().getProperty("path.master.sales.out");// path.master.customer.out");
			fileNameFull = ExportHelper.genFileName(tableBean, userBean);
			logger.info("connection timeout:" + ftp.getConnectTimeout());
			...
			if (dataExportSt != null && !Utils.isNull(dataExportSt.toString()).equals("")) {
				if ("".equalsIgnoreCase(path)) {
					// path = tableBean.getExportPath();
				}
				logger.info("Write To Path:" + path);
				...
				out = ftp.storeFileStream(fileNameFull);

				/** new Code ******************************************************/
				logger.info("Start Write OutputStreamWriter data:");
				w = new BufferedWriter(new OutputStreamWriter(out, Constants.FTP_EXPORT_TO_ORACLE_ENCODING_TIS_620));
				w.write(dataExportSt.toString());
				w.flush();
				w.close();
				...
				try {
					logger.info(" Start CHMOD 777 ");
					ftp.sendSiteCommand("CHMOD 777 " + fileNameFull);
					logger.info("End CHMOD 777 FTP sendSiteCommand Response " + ftp.getControlEncoding() + " :"
							+ ftp.getReplyString());
				} catch (Exception e) {
					logger.error(e.getMessage(), e);
	 ...
	 * @throws Exception
	 */
	public void deleteAllFileInFTPCaseRollback(LinkedHashMap<String, TableBean> controlTableMap, String path)
			throws Exception {
		FTPClient ftp = null;
		try {
			ftp = new FTPClient();
	...
	public String uploadFileToFTPXX(TableBean tableBean, String path, String encoding) throws Exception {
		...
		try {
			...
			if (tableBean.getDataStrExport() != null
				...
				/*
				 * out.write(tableBean.getDataStrExport().toString().getBytes()); out.flush();
				 * out.close();
				 */
	 ...
	 * @param destination
	 * @param controlTableMap
	 * @throws Exception Step Move file and Delete from Source File
	 */
	public void moveFileFTP(String source, String destination, LinkedHashMap<String, TableBean> controlTableMap)
			throws Exception {
		String ftpResponse = "";
	 ...
	 * @throws Exception Step Move file and Delete from Source File
	 */
	public void moveFileFTP_NEW(String source, String destination, List<FTPFileBean> fileImportSuccessList)
			throws Exception {
		FTPClient ftp = null;
		try {
			logger.debug("Move File From :" + source + " TO :" + destination);
	...
	public void moveFileFTP(String source, String destination, List<FileImportTransBean> fileImportSuccessList)
			throws Exception {
		FTPClient ftp = null;
		try {
			logger.debug("Move File From :" + source + " TO :" + destination);
	...
	public void createFolderFTP(String rootPath, String newFolder) throws Exception {
		...
		try {
			...
			// System.out.println("Step1 FTP Response "+ftp.getControlEncoding()+"
			// :"+ftp.getReplyString());
	...
	public void deleteFileFTP(String source, LinkedHashMap<String, TableBean> controlTableMap) throws Exception {
		...
		try {
			...
			for (int i = 1; it.hasNext(); i++) {
				...
				ftp.deleteFile(tableBean.getFileFtpNameFull());
				logger.debug("Delete FTP Response fileName: " + tableBean.getFileFtpNameFull() + ":"
						+ ftp.getControlEncoding() + " :" + ftpResponse);
			} // for
	 ...
	 * @throws Exception
	 */
	public void uploadBackUpDBFileToFTP(User user, String parentPath, String fileName, StringBuffer dataFiles)
			throws Exception {
		FTPClient ftp = null;
		File file = null;
		...
		try {
			...
			// logger.debug("FTP sendSiteCommand Response "+ftp.getControlEncoding()+"
			// :"+ftp.getReplyString());
		} catch (SocketException e) {
			throw new FTPException("Could not connect to FTP server");
		} catch (UnknownHostException e) {
	...
	//(String rootFtpPath, String ftpFilePath, String localFile)

	public void uploadBackUpDBZipFileToFTP(String rootFtpPath, String ftpFilePath, String localFile)
			throws Exception {
		FTPClient ftp = null;
		File file = null;
		InputStream fis = null;
		// int CON_TIMEOUT = (int) TimeUnit.SECONDS.toMillis(20); // fail if can't
		// connect within 20 seconds
		// int LIVE_TIMEOUT = (int) TimeUnit.MINUTES.toMillis(5); // allow up to 5
		// minutes for data transfers
		try {
			ftp = new FTPClient();
			ftp.setControlEncoding(Constants.FTP_EXPORT_TO_ORACLE_ENCODING_TIS_620);
			ftp.connect(server);
			...
			////////////////////////////////////////////
			logger.debug("Parent path:" + rootFtpPath);
			...
			/*
			 * logger.debug("upload ftp status :" + uploadOk); if (!uploadOk) {
			...
			// Upload file
			/*
			 * fis = new FileInputStream(localFile); OutputStream out =
			 * ftp.storeFileStream(fileName); byte buf[] = new byte[8192]; while
			 * (fis.read(buf) != -1) { out.write(buf); } fis.close(); out.close();
			 */
			 ...
			 * 
			 * byte[] b = new byte[4096]; while (fis.read(b) != -1) { out.write(b); }
	...
	/** Code Upload DBBACKUP OLD CODE **/
	public void uploadBackUpDBZipFileToFTP_BK(String rootFtpPath, String ftpFilePath, String localFile)
			throws Exception {
		...
		try {
			/*
			 * ftp = new FtpClient(server, 21); ftp.login(userFtp, passwordFtp);
			 * ftp.binary();
			 * 
			 * fos = new FileInputStream(localFile); tos = ftp.put(ftpFilePath); dos = new
			 * DataOutputStream(tos);
			 * 
			 * byte[] buffer = new byte[2048 * 2048]; for (int length; (length =
			 * fos.read(buffer)) > 0;) { dos.write(buffer, 0, length); } tos.flush();
			 * dos.flush(); logger.info("upload success");
			 * 
