...
/**
 * StockDiscountDocumentProcess Class
 * 
 * @author Witty
 * @version 1.0
 * 
 * 
 */
public class StockDiscountDocumentProcess extends DocumentSequenceProcess {


	
	/**
	 * Get Next DocNo
	 * "D"+ Salecode +  YY + MM + Running 2 หลัก  เช่น S001 5810 01  เป็นต้น
	 */
	public String getNextDocumentNo(Date requestDate,String salesCode, int activeUserID, Connection conn)
			throws Exception {
		
		int seq = getNextSeqStockDiscount(requestDate,salesCode, STOCK_DISCOUNT_NUMBER, activeUserID); 
		
		// String docNo = "O";
		String docNo = "D"+salesCode;
...
public boolean checkCodeDuplicate(String code,Connection conn) throws Exception {
		...
		try {
			...
			String sql = "SELECT COUNT(*) as TOT FROM t_stock_discount ";
...
protected int getNextSeqStockDiscount(Date requestDate,String salesCode, int docTypeId, int activeUserID) throws Exception {
	...
	try {
		...
		if (seq == null){
			...
			docSeq.setStartNo(1);
			docSeq.setCurrentNext(getCurrentNextStockDiscount(conn,salesCode,docSeq.getCurrentMonth(),docSeq.getCurrentYear()));
			...
			if (newSeq) {
				...
				seq[0].setCurrentNext(getCurrentNextStockDiscount(conn,salesCode,seq[0].getCurrentMonth(),seq[0].getCurrentYear()));// getCurrentNextSeq
...
//Request_number =DS00158100001
protected int getCurrentNextStockDiscount(Connection conn ,String salesCode,String currentMonth,String currentYear) throws Exception {
	int nextValue = 0;
	...
	try {
		...
		whereCause.append("\n CONVERT(substring(request_number,10,4),UNSIGNED INTEGER)  as currentSeq ");
		whereCause.append("\n from t_stock_discount ");
		whereCause.append("\n where 1=1 ");
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			init.init(conn);
			System.out.println(new StockDiscountDocumentProcess().getNextDocumentNo(new Date(),"VAN001", 1, conn));
