...
public class PageingGenerate {
	...
	//Gen Paging
		public static StringBuffer genPageingMobileModel(int totalPage,int totalRecord,int currPage, int startRec,int endRec,int no){
			...
			try{ 
				   ...
				   h.append("<div class='row mb-1'> \n");
				    h.append("<span> รายการทั้งหมด  "+totalRecord +" รายการ </span>");
				    h.append("<span>, หน้า "+currPage +"/"+totalPage +"</span>\n");
				   h.append("</div> \n");
		...
		public static StringBuffer genTripPageing(List<Customer> tripList,int curPage){
			StringBuffer h = new StringBuffer("");
			try {
				 h.append("<div class='container'> \n");
				 h.append("  <div class='row mb-1'> \n");
				 h.append("     <span> แสดงรายการ Trip </span>");
					/* h.append("     <span> จุด</span>\n"); */
				 h.append("   </div> \n");
					

				 List<Customer> its= tripList;
			     for(int i=0;i<its.size();i++){
			    	int tripNo =Integer.parseInt(its.get(i).getTripDay());
			    	if( curPage != tripNo){
			    		h.append("<span class='pagination'><a href=javascript:gotoPage('"+(tripNo)+"')");  
					    h.append(" title='Go to page "+(tripNo)+"'> "+(tripNo) +"</a></span>");
				  }else{
					  h.append("<!-- Active Current Page -->");
				      h.append("<span class='pagination'><a href='#' class='active'>"+(tripNo) +"</a></span>");
				  } 
			     } 
				 h.append(" </div> \n");
			}catch(Exception e) {
