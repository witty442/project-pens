...
function save(path,moveOrderType){
	if(checkTableCanSave()){
		...
		for(var index=1;index<=itemCode.length;index++){
			if(itemCode[index-1].value !='' && status[index-1].value !='DELETE'){
			   sumTotalInRow(index);
			}//if   
		}//for
	...
	for(var i=0;i<itemCode.length;i++){
	 // alert(itemCode[i].value+":"+status[i].value+":"+arInvoiceNo[i].value);
	   if(itemCode[i].value !='' && status[i].value !='DELETE'){
	      ...
	      expireDate[i].className ='normalTextInput';
		 ...
		 // alert("["+qty[i].value+"]["+sub[i].value+"]["+expireDate[i].value+"]");
		  ...
		  if( reason[i].value =="EXPIRED PRODUCT" && expireDate[i].value =='' ){
			  expireDate[i].className ='errorTextInput';
			  error = true;
		  }
...
function sumTotalInRow(index){
	...
	if(productCode[index-1].value != '' && arInvoiceNo[index-1].value != ''
		...
		console.log("uom1Price:"+uom1Price[index-1]);
