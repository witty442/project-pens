...
function open_bill(path, rowNo) {
	...
	var objInvoiceId = document.getElementsByName('bill.invoiceId');
	for(var i=0;i<objInvoiceId.length;i++)
	{
		selectedInvoiceId+=','+objInvoiceId[i].value;
	}
	if(selectedInvoiceId.length>0)
		selectedInvoiceId = selectedInvoiceId.substring(1, selectedInvoiceId.length);
	
	var customerId = document.getElementsByName('receipt.customerId')[0].value;
	var oracleCustId = document.getElementsByName('receipt.oracleCustId')[0].value;//=customer_id(oracle)
	var billToAddressId = document.getElementsByName('receipt.billToAddressId')[0].value;
	
	if (rowNo == null){
		window.open(path + "/jsp/pop/billPopup2.jsp?selectedInvoiceId="+selectedInvoiceId+"&cust="+customerId+"&oracleCustId="+oracleCustId+"&billToAddressId="+billToAddressId, "Bill",
			"width="+screen_width+",height=400,location=0");
	}else{
		window.open(path + "/jsp/pop/billPopup2.jsp?row=" + rowNo+"&selectedInvoiceId="+selectedInvoiceId+"&cust="+customerId+"&oracleCustId="+oracleCustId+"&billToAddressId="+billToAddressId, "Bill",
			"width="+screen_width+",height=400,location=0");
	}
	return;
}
...
function applyBill(path, rowNo, type , recAmount, seedId) {
	...
	var customerId = document.getElementsByName('receipt.customerId')[0].value;
	var oracleCustId = document.getElementsByName('receipt.oracleCustId')[0].value;
	var billToAddressId = document.getElementsByName('receipt.billToAddressId')[0].value;
	...
	var param = "";
	if (rowNo == null){
		param  = "selectedInvoiceId="+selectedInvoiceId+"&rec="+recAmount+"&type="+type;
	    param +="&seed="+seedId+"&cust="+customerId+"&cns="+cns+"&writeOff="+value;
	    param +="&billToAddressId="+billToAddressId;
	    param +="&oracleCustId="+oracleCustId;
	    
		window.open(path + "/jsp/pop/applyBillPopup.jsp?"+encodeURI(param), "ApplyBill",
			"width="+screen_width+",height=400,location=No,resizable=No");
	}else{
		param  ="row=" + rowNo+"&selectedInvoiceId="+selectedInvoiceId+"&rec="+recAmount+"&type="+type;
		param +="&seed="+seedId+"&cust="+customerId+"&cns="+cns+"&writeOff="+value;
		param +="&billToAddressId="+billToAddressId;
		param +="&oracleCustId="+oracleCustId;
...
function fillApply(seed,allbill,allpaid){
	...
	for(var i=0;i<seedObjs.length;i++){
		if(seedObjs[i].value==seed){
			if(allbill.length>0){
			   allbill=allbill.substring(1,allbill.length);
			}
			if(allpaid.length>0){
			   allpaid=allpaid.substring(1,allpaid.length);
			 }
