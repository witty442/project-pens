...
function validateVanPaymentMethod(){
	...
	if(vanPaymentMethod.value =='CREDIT'){
		<%if(receiptCreditFlag.equalsIgnoreCase("0")){%>
		     document.getElementById("error-dialog-content").innerHTML  = "ร้านค้านี้ ยังไม่ได้เปิดสิทธิ์ ให้ขายเชื่อ ได้ กรุณาแจ้งทางผู้จัดการหรือซุปเปอร์ ให้ทำเรื่องเปิดสิทธิ์การขายเชื่อ";
		     $("#brand-dialog").modal({backdrop: 'static', keyboard: true,show:true}) ; 
		     r = false;
		<%}if(receiptCreditFlag.equalsIgnoreCase("-1")){%>
		    document.getElementById("error-dialog-content").innerHTML  = "ร้านค้านี้ ยังมีบิลเก่า ที่ยังไม่ได้เก็บเงิน  ต้องเก็บเงินบิลเก่าก่อน จึงสามารถเปิดบิลขายเชื่อใหม่";
		    $("#brand-dialog").modal({backdrop: 'static', keyboard: true,show:true}) ; 
	        r = false;
		<%}%>
	}
...
function validateVanCreditLimit(){
	...
	if(vanPaymentMethod.value =='CREDIT'){
		...
		if(parseFloat(netAmount.value) > parseFloat(custCreditLimit.value)){
		   ...
		   document.getElementById("error-dialog-content").innerHTML  = "ยอดเงินที่ขาย เกินวงเงินที่กำหนด ไม่สามารถทำรายการขายได้ วงเงินที่ขายได้ของร้านนี้ ("+custCreditLimit.value+")";
		   $("#brand-dialog").modal({backdrop: 'static', keyboard: true,show:true}) ; 
		   r = false;
		}
	}
