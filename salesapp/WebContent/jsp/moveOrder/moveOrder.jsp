...
List<PD> pdList = MPD.getPDList(user);
pageContext.setAttribute("pdList",pdList,PageContext.PAGE_SCOPE);
...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/moveOrder.js?v=<%=SessionGen.getInstance().getIdSession()%>"></script>
...
function addProductToSalesOrder(){
	...
	$(function(){
		var getData = $.ajax({
			url: "${pageContext.request.contextPath}/jsp/moveOrder/moveOrderAddProductToSO.jsp",
...
function addProductToBasket(){
	...
	for(var i =0;i < lineAmts.length; i++){
		...
		$(function(){
			var getData = $.ajax({
				url: "${pageContext.request.contextPath}/jsp/moveOrder/moveOrderAddProductToBasket.jsp",
				data : "custId=" +custId
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
	    	...
	    	<!-- PROGRAM HEADER -->
	      	<jsp:include page="../program.jsp">
				<jsp:param name="function" value="${moveOrderForm.moveOrder.moveOrderType}"/>
		            ...
		            <td bgcolor="#f8f8f8">
						...
						<table align="center" border="0" cellpadding="3" cellspacing="0" class="body">
							<tr>
								...
								<c:if test="${moveOrderForm.moveOrder.showSaveBtn =='true'}">
								   <c:if test="${moveOrderForm.moveOrder.canEdit =='true'}">
									<a href="#" onclick="return save('${pageContext.request.contextPath}','${moveOrderForm.moveOrder.moveOrderType}');">
									  <input type="button" value="บันทึกรายการ" class="newPosBtnLong">
									</a>	
								   </c:if>	
								</c:if>		
								<c:if test="${moveOrderForm.moveOrder.showPrintBtn =='true'}">		
								  		
								     <a href="#" onclick="printMoveOrder('${pageContext.request.contextPath}','${moveOrderForm.moveOrder.moveOrderType}');">
									  <input type="button" value="พิมพ์ใบเบิก" class="newPosBtnLong">
								    </a>
							
								</c:if>
                                <c:if test="${moveOrderForm.moveOrder.showCancelBtn =='true'}">
                                   <c:if test="${moveOrderForm.moveOrder.canEdit =='true'}">
										<a href="#" onclick="cancelMoveOrder('${pageContext.request.contextPath}','${moveOrderForm.moveOrder.moveOrderType}');"">
						...
						<!-- Hidden Field -->
						<html:hidden property="moveOrder.priceListId" styleId="moveOrder.priceListId"/>
