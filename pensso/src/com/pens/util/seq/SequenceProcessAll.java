...
/**
 * SequenceProcessAll Class (PENSSO.C_SEQUENCE_ALL)
 * 
 * @author Witty
...
public class SequenceProcessAll {
	...
	public synchronized BigDecimal getNextValueModel(Connection conn,String sequenceType) throws Exception {
		...
		try {
			...
			sql.append("SELECT seq  FROM PENSSO.C_SEQUENCE_ALL WHERE SEQUENCE_TYPE ='"+sequenceType+"'");
			...
			if (rst.next()) {
				...
				//update nextValue
				stmt = conn.createStatement();
				stmt.executeUpdate("UPDATE PENSSO.C_SEQUENCE_ALL SET SEQ ="+(nextValue.add(new BigDecimal("1")))+" WHERE SEQUENCE_TYPE ='"+sequenceType+"'");
			} else{
				nextValue = new BigDecimal("1");
				//not found -> insert
				stmt = conn.createStatement();
				stmt.executeUpdate("INSERT INTO PENSSO.C_SEQUENCE_ALL(SEQUENCE_TYPE,SEQ)VALUES('"+sequenceType+"',"+(nextValue.add(new BigDecimal("1")))+")");
			}
		} catch (Exception e) {
			throw e;
		} finally {
	...
	public static synchronized BigDecimal getNextValueModel(Connection conn,String sequenceType,Date dateObj) throws Exception {
		...
		try {
			...
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT seq  FROM PENSSO.C_SEQUENCE_ALL WHERE SEQUENCE_TYPE ='"+sequenceType+"'  AND MONTH='"+curMonth+"' AND YEAR='"+curYear+"'");
			stmt = conn.createStatement();
			logger.debug(sql.toString());
			rst = stmt.executeQuery(sql.toString());
			if (rst.next()) {
				...
				//update nextValue
				stmt = conn.createStatement();
				stmt.executeUpdate("UPDATE PENSSO.C_SEQUENCE_ALL SET SEQ ="+(nextValue.add(new BigDecimal("1")))+" WHERE SEQUENCE_TYPE ='"+sequenceType+"' AND MONTH='"+curMonth+"' AND YEAR='"+curYear+"'");
			} else{
				logger.debug("Insert PENSBME_C_SEQUENCE");
				nextValue = new BigDecimal("1");
				//not found -> insert
				stmt = conn.createStatement();
				stmt.executeUpdate("INSERT INTO PENSSO.C_SEQUENCE_ALL(SEQUENCE_TYPE,MONTH,YEAR,SEQ)VALUES('"+sequenceType+"','"+curMonth+"','"+curYear+"',"+(nextValue.add(new BigDecimal("1")))+")");
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
