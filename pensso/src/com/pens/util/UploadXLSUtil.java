...
public class UploadXLSUtil {
	...
	public ArrayList loadXls(FormFile dataFile, int sheetNo, int rowNo, int maxRowNo, int maxColumnNo, int keyColumn)
		...
		try {
			// InputStream data = dataFile.getInputStream();
			...
			while (true) {
				...
				if (maxRowNo > 0 && rowNo == maxRowNo) {
					break;
				}

				if (row != null && row.getCell(keyColumn) != null) {
					logger.debug("select row no.: " + (row.getRowNum() + 1));
	...
	private Object getCellValue(int colNo, Cell cell) throws Exception {
