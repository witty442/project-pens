 ...
 * 
 * @author Witty.b
 * @version $Id: OrderSpecialAction.java,v 1.0 07/09/2018 00:00:00  $

 * 
 *          atiz.b : set current sales reps
 */
public class OrderSpecialAction extends I_Action {
	...
	protected String prepare(ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if(User.VAN.equals(user.getType())){
				canSaveCasePayPrevBill = OrderSpecialUtils.canSaveCreditVan(conn,user, customerId);
	...
	protected String prepare(String id, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<OrderLine> lines = new OrderSpecialProcess().fillLinesShow(lstLines);
	...
	public ActionForward prepareEditOrder(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if(User.VAN.equals(user.getType())){
				boolean canSaveCasePayPrevBill = OrderSpecialUtils.canSaveCreditVan(conn,user, order.getCustomerId());
			...
			//Split 2 Line
			orderForm.setLines(new OrderSpecialProcess().fillLinesSave(orderForm.getLines(), user, (String) request.getSession().getAttribute("memberVIP")));
	...
	public ActionForward preSave(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		Connection conn = null;
		OrderSpecialProcess orderSpecialProcess = new OrderSpecialProcess();
		try {
			OrderSpecialForm orderForm = (OrderSpecialForm) form;
			conn = new DBCPConnectionProvider().getConnection(conn);
			...
			orderForm.setLines(orderSpecialProcess.fillLinesSave(orderForm.getLines(), userActive, (String) request.getSession().getAttribute("memberVIP")));
            
			/******* Add Promotion Line(Y) only From orderFrom to promotionLine *********************/
			for (OrderLine line : orderForm.getLines()) {
				if (line.getPromotion().equalsIgnoreCase("Y")) {
					promotionLine.add(line);
				}
			...
			promotionSLine =  orderSpecialProcess.fillLinesShowPromotion(promotionSLine);
			//debug
			/*for (OrderLine line : promotionSLine) {
				if (line.getPromotion().equalsIgnoreCase("S") ) {
			...
			//SumQtySameProduct 'S'
			promotionSLine = orderSpecialProcess.sumQtyProductPromotionDuplicate(promotionSLine);
			/*********************Promotion Special****************************************************/
			...
			List<OrderLine> odLines = orderSpecialProcess.fillLinesShow(orderForm.getLines());

			//add Line Blank UOM (1 or 2)
			odLines = new OrderSpecialProcess().fillLinesShowBlankUOM(conn,String.valueOf(orderForm.getOrder().getPriceListId()),odLines);
			
			//logger.info("Debug before promotion");
			//new OrderSpecialProcess().debug(modProcess.getAddLines());
			...
			if(exeOrderProcessfillLinesShowPromotion){
				promotionLines = orderSpecialProcess.fillLinesShowPromotion(modProcess.getAddLines());
			}else{
				  // default 1
		        promotionLines = orderSpecialProcess.fillLinesShow(modProcess.getAddLines());
			}
			
			/** sum(qty1,qty2) Duplicate product promotion **/
			promotionLines = orderSpecialProcess.sumQtyProductPromotionDuplicate(promotionLines);

			//logger.info("Debug after promotion");
			//new OrderSpecialProcess().debug(promotionLines);
			...
			if(User.TT.equals(userActive.getType()) ){
				...
				if(productInvalid.length() >0){
					...
					request.setAttribute("Message","ไม่สามารถบันทึกข้อมูลได้     มีบางสินค้าที่ระบุ ต้องบันทึกแยก Order" );
			...
			//debug 
			logger.debug("lines size:"+orderForm.getLines().size());
		} catch (Exception e) {
			e.printStackTrace();
			request.setAttribute("Message", InitialMessages.getMessages().get(Messages.FETAL_ERROR).getDesc()
					+ e.getMessage());
	...
	public ActionForward backToAdd(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if(User.VAN.equals(userActive.getType())){
				boolean canSaveCasePayPrevBill = OrderSpecialUtils.canSaveCreditVan(conn,userActive, orderForm.getOrder().getCustomerId());
				String canReceiptChequeFlag = CustomerReceiptFilterUtils.canReceiptCheque(conn,orderForm.getOrder().getCustomerId());
				String canReceiptCreditFlag = CustomerReceiptFilterUtils.canReceiptCredit(conn,orderForm.getOrder().getCustomerId());
			...
			orderForm.setLines(new OrderSpecialProcess().fillLinesSave(orderForm.getLines(), userActive, (String) request
	...
	protected String save(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			for (OrderLine line : orderForm.getLines()) {
				...
				if (line.getPromotion().equalsIgnoreCase("S") ) {
					...
					if (line.getPromotion().equalsIgnoreCase("S") ) {
						...
						logger.debug("productCode:"+line.getProduct().getCode());
						logger.debug("taxable:"+line.getTaxable());
						logger.debug("qty:"+line.getQty()+","+line.getQty1()+","+line.getQty2());
						logger.debug("price:"+line.getPrice()+","+line.getPrice1()+","+line.getPrice2());
						logger.debug("lineAmount:"+line.getLineAmount()+","+line.getLineAmount1()+","+line.getLineAmount2());
						logger.debug("discount:"+line.getDiscount()+","+line.getDiscount1()+","+line.getDiscount2());
						logger.debug("totalAmount:"+line.getTotalAmount()+","+line.getTotalAmount1()+","+line.getTotalAmount2());
						logger.debug("*******************************************************");
					}
				}
				//Save Line to DB
			...
			if (userActive.getRole().getKey().equals(User.VAN) 
				 ...
				 new OrderSpecialProcess().createAutoReceipt(orderForm.getAutoReceipt(), order, orderForm.getLines(), receiptByList, null, userActive, conn);
				 //set msg 
			...
			/*for (OrderLine line : orderForm.getLines()) {
				if( Utils.isNull(line.getPromotion()).equalsIgnoreCase("S")){
					...
					line.setLineAmount(line.getLineAmount1()+line.getLineAmount2());
					
					line.setDiscount1((-1)*line.getLineAmount1());
	...
	public ActionForward createAutoReceipt(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
	       ...
	       if(Utils.isNull(request.getParameter("actionSave")).equalsIgnoreCase("saveAutoReceipt")){
				...
				if (!new OrderSpecialProcess().createAutoReceipt(orderForm.getAutoReceipt(), order, lstLines, orderForm.getBys(),
	...
	protected String search(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			OrderSpecialCriteria criteria = getSearchCriteria(request, orderForm.getCriteria(), this.getClass().toString());
			orderForm.setCriteria(criteria);
			String whereCause = "";
	...
	protected void setNewCriteria(ActionForm form) throws Exception {
		...
		if (!orderForm.getOrder().getOrderType().equalsIgnoreCase(Customer.DIREC_DELIVERY)) {
			// VAN && TT
			Customer customer = new MCustomer().find(String.valueOf(orderForm.getOrder().getCustomerId()));
			orderForm.setCriteria(new OrderSpecialCriteria());
			orderForm.getOrder().setCustomerId(customer.getId());
			orderForm.getOrder().setCustomerName((customer.getCode() + "-" + customer.getName()).trim());
		} 
	...
	public ActionForward printReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		OrderSpecialForm reportForm = (OrderSpecialForm) form;
		...
		try {
			...
			reportForm.setLines(new OrderSpecialProcess().fillLinesShow(lines));
	...
	public ActionForward printListOrderProductReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try{
			...
			orderForm = (OrderSpecialForm)request.getSession().getAttribute("order_from_to_print");
			/**Debug **/
