...
public class BatchTaskDAO {
	...
	public MonitorItemBean insertMonitorItemDetail(Connection conn ,MonitorItemDetailBean[] monitorItemDetailBean ) throws Exception {
		...
		try {
			...
			" type ," +
			" amount )" +
			" VALUES ((select max(id) from PENSSO.monitor_item),?,?,?,?,?) ";
    ...
    public static void clearTaskControlMonitorAll() {
    	...
    	try {
            conn = DBConnection.getInstance().getConnection();
	       	String sql = "UPDATE PENSSO.c_monitor SET  transaction_id = 0";
			ps = conn.prepareStatement(sql);
            ps.executeUpdate();
    ...
    public void updateControlMonitor(Connection conn,BigDecimal transactionId,String action) throws Exception {
		PreparedStatement ps = null;
		try {
			String sql = "UPDATE PENSSO.c_monitor SET  transaction_id = "+transactionId+" WHERE action = '"+action+"'";
			...
			if(r==0){
				//insert 
				sql = "insert into PENSSO.c_monitor(action,transaction_id)values('"+action+"',"+transactionId+")";
	...
	public  MonitorBean[] findMonitorListNew(User user,String name) throws Exception{
		...
		try{
			...
			sql.append(" , (select max(id) from PENSSO.monitor_item where monitor_item.monitor_id = monitor.monitor_id)  as monitor_item_id  \n");
	...
	public  MonitorItemBean findMonitorItemBeanByPK(User user, String id) throws Exception{
		...
		try{
			...
			sql.append(" from PENSSO.monitor_item where 1=1 \n");
	...
	public  List<MonitorItemResultBean> findMonitorItemResultList(Connection conn ,BigDecimal monitorIItemId,String statusDesc) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select * from PENSSO.monitor_item_result where 1=1 \n");
	...
	public String findMonitorItemColumnHeadResultList(Connection conn ,BigDecimal monitorIItemId) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" select * from PENSSO.MONITOR_ITEM_COLUMN_RESULT where 1=1 \n");
			sql.append(" and monitor_item_id ="+monitorIItemId +"\n");
		    logger.debug("SQL:"+sql.toString());
