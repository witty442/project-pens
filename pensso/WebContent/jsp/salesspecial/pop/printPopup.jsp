...
function loadMe(_path){
   ...
   <%
	if("list_order_product".equals(request.getParameter("report_name"))){
		...
		document.orderSpecialForm.action = _path + "/jsp/saleOrderSpecialAction.do?do=printListOrderProductReport&customerId=<%=customerId%>";
  ...
  <%}else if("tax_invoice_summary".equals(request.getParameter("report_name"))){ 
       ...
       if("bill".equalsIgnoreCase(reportType) ){
    	  if(vanCanPrintBill){
    		  msg = "";
  %>
             document.orderSpecialForm.action = _path + "/jsp/saleOrderSpecialAction.do?do=printReportSummary&orderId=<%=orderId%>&fileType=PRINTER&reportType=<%=reportType%>";
	         document.orderSpecialForm.submit();

	     <%}else{
	    	 msg = "กรุณาบันทึกรับเงินเชื่อก่อน พิมพ์ใบเสร็จรับเงิน";
	     %>
	      // alert("กรุณาบันทึกรับเงินเชื่อก่อน พิมพ์ใบเสร็จรับเงิน");
	       //Goto Page PD_Receipt
	       
	  <%}}else{%>
	      //Other Report No check
	      document.orderSpecialForm.action = _path + "/jsp/saleOrderAction.do?do=printReportSummary&orderId=<%=orderId%>&fileType=PRINTER&reportType=<%=reportType%>";
          document.orderSpecialForm.submit();
	  <%}%>
...
//report LQ300 Receipt_TaxInvoice first report
  ...
  %>
	   ...
	   <%}%>
       document.orderSpecialForm.action = _path + "/jsp/saleOrderSpecialAction.do?"+param;
	   document.orderSpecialForm.submit();
