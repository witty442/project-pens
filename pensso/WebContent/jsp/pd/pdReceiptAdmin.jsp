...
<script type="text/javascript">
	...
	function search(path){
		document.pdReceiptForm.action = path + "/jsp/pdReceipt.do?do=searchAdmin";
		document.pdReceiptForm.submit();
		return true;
	}

	function clearForm(path){
		document.pdReceiptForm.action = path + "/jsp/pdReceipt.do?do=clearFormAdmin";
		document.pdReceiptForm.submit();
		return true;
	}

	function cancelPDReceipt(orderNo){
		if(!confirm('ต้องการยกเลิกรายการบันทึกรับเงินเชื่อ \r\nท่านแน่ใจหรือไม่?')){return false;}
		document.pdReceiptForm.action='${pageContext.request.contextPath}/jsp/pdReceipt.do?do=saveAdmin&orderNo='+orderNo;
		document.pdReceiptForm.submit();
	}
		...
		<tr id="framerow">
			...
			<td
				...
				<jsp:include page="../program.jsp">
					<jsp:param name="function" value="ManageCreditReceiptAdmin" />
							...
							<!-- BODY --> <html:form action="/jsp/pdReceipt">
								...
								<table align="center" border="0" cellpadding="3" cellspacing="0"
									...
									<tr>
										...
										<b>จัดการรายการบันทึกรับเงินเชื่อ(Admin) ยกเลิกรายการที่มีการบันทึกรับเงินเชื่อไปแล้ว เฉพาะรายการที่ยังไม่ ส่งข้อมูลเข้าส่วนกลาง</b>
										</font>
										</td>
										...
										<c:forEach var="results" items="${pdReceiptForm.pdReceipts}"
											...
											<tr class="<c:out value='${tabclass}'/>">
												...
												<td class="td_text_center" width="10%">
													...
													<c:choose>
														<c:when test="${results.paymentMethod} == 'CH'}">
															เช็ค
														</c:when>
														<c:otherwise>
														    เงินสด
												...
												<td class="td_text_center" width="10%">
												    <a href="javascript:cancelPDReceipt('${results.receiptNo}');">
