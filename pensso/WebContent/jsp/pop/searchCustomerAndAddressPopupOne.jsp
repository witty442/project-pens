...
function selectOneRadio(){
	...
	for(var i=0;i<chRadio.length;i++){
        if(chRadio[i].checked){
            ...
            window.opener.setStoreMainValue(code[i].value,desc[i].value,address[i].value,customerId[i].value);
        	window.close();
            break;
        }
	}
...
<display:table  id="item" name="requestScope.CUSTOMER_LIST" 
    ...
    <display:column  title="เลือกข้อมูล"   sortable="false" class="chk">
		...
		<input type ="hidden" name="address" id="address" value="<bean:write name="item" property="address"/>" />
		<input type ="hidden" name="customerId" id="customerId" value="<bean:write name="item" property="customerId"/>" />
	 </display:column>
    ...
    <display:column  title="ที่อยู่" property="address"  sortable="false" class="desc"/>								
</display:table>	
<!-- RESULT -->
