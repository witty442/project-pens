function search(path){
	document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=search&rf=Y";
	document.moveOrderForm.submit();
	return true;
}

function createNewMoveOrder(path,moveOrderType){
	document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=createNewMoveOrder&moveOrderType="+moveOrderType;
	document.moveOrderForm.submit();
	return true;
}

function editMoveOrder(path,moveOrderType,requestNumber){
	//alert(moveOrderType+":"+requestNumber);
	document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=editMoveOrder&moveOrderType="+moveOrderType+"&requestNumber="+requestNumber;
	document.moveOrderForm.submit();
	return true;
}

function printMoveOrder(path,moveOrderType){

    document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=printMoveOrder";
    document.moveOrderForm.submit();
    return true;
}

function popupPrint(path,moveOrderType,requestNumber){
	var param = "&moveOrderType="+moveOrderType;
	    param += "&requestNumber="+requestNumber;
        //window.open(path + "/jsp/moveOrderAction.do?do=popupPrint"+param, "Print", "width=80,height=50,location=No,resizable=No");
	
	 document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=popupPrint"+param;
	 document.moveOrderForm.submit();
	 return true;
}

function cancelMoveOrder(path,moveOrderType){
	//alert(moveOrderType+":"+requestNumber);
...
function viewMoveOrder(path,moveOrderType,requestNumber){
	document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=viewMoveOrder&moveOrderType="+moveOrderType+"&requestNumber="+requestNumber;
	document.moveOrderForm.submit();
	return true;
}

function save(path,moveOrderType){
	var requestDate = document.getElementsByName('moveOrder.requestDate')[0];
	var pdCode = document.getElementsByName('moveOrder.pdCode')[0];
    ...
    var requestDate = document.getElementsByName('moveOrder.requestDate')[0].value;
	...
	if(reqMonth != currMonth || reqYear != currYear){
		...
		document.getElementsByName('moveOrder.requestDate')[0].focus();
		return false;
	}
...
function backsearch(path,moveOrderType) {
	...
	document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=prepare"+"&action=back&moveOrderType="+moveOrderType;
	document.moveOrderForm.submit();
}

function clearForm(path){
	document.moveOrderForm.action = path + "/jsp/moveOrderAction.do?do=clear";
	document.moveOrderForm.submit();
	return true;
}
