...
function addProduct(path,objValue){
	...
	if(index < 0){
		...
		document.getElementsByName('lines.isPromotionSpecial')[index].value = objValue.isPromotionSpecial;
...
function findDupIndex(tbl,objValue){
	...
	for(i=0;i<tbl.rows.length;i++){
		if(i!=0){
			if(objValue.isPromotionSpecial =='Y'){
...
function setValueToProduct(path, objValue){
	...
	inputLabel+="<input type='hidden' name='lines.isPromotionSpecial' value='Y'>";//Y only this page for promotion special
	...
	tbl.rows[objValue.row].cells[c++].innerHTML= "<img border=0 src='"+path+"/icons/check.gif'/>";//is promotion special
...
function createProductList(){
	...
	//New case special
	var isPromotionSpecial=document.getElementsByName('lines.isPromotionSpecial');
	...
	for(var i=0;i<ids.length;i++){
		...
		inputLabel+="<input type='text' name='lines["+i+"].isPromotionSpecial' value='"+isPromotionSpecial[i].value+"'>";
		inputLabel+="<hr/>";
...
function calculatePrice(){
	...
	for(var i=0;i<totals1.length;i++){
		...
		vatLine=0;
		totalAmtInVat=0;
		//case product promotion special no calculate
		if(isPromotionSpecial[i].value=='Y'){
			//no calculate
		}else{
