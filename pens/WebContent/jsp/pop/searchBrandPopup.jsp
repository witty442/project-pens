...
function selectOneRadio(){
	...
	for(var i=0;i<chRadio.length;i++){
        if(chRadio[i].checked){
            ...
            window.opener.setBrandValue(code[i].value,desc[i].value);
        	window.close();
            break;
        }
	}
...
<!-- RESULT -->
<display:table  id="item" name="requestScope.BRAND_LIST" 
    ...
    <display:column  title="รหัสแบรนด์" property="code"   sortable="false" class="code"/>
