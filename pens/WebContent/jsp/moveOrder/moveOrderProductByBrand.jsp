...
try{	
	if(categoryCode != null && categoryCode.length()>0){
		catalogs = new MProduct().getMoveOrderProductCatalogByBrand(categoryCode,orderDate,priceListId);
	}
}catch(Exception e){
	e.printStackTrace();
}
...
%>
<tr>
<td><%=catalog.getProductCode()%></td>
<td><%=catalog.getProductNameDisplay()%></td>
<td><%=catalog.getUom1()%> &frasl; <%=catalog.getUom2()%></td>
<td><%=formatter.format(catalog.getPrice1())%> &frasl; <%=formatter.format(catalog.getPrice2())%>
</td>
