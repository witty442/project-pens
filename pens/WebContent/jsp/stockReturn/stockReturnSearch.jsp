...
function clearForm(path) {
	document.stockReturnForm.action = path + "/jsp/stockReturnAction.do?do=prepare"+"&action=new";//clearForm
	document.stockReturnForm.submit();
}

function search(path){
	document.stockReturnForm.action = path + "/jsp/stockReturnAction.do?do=search&action=new";
	document.stockReturnForm.submit();
	return true;
}
...
function viewStockReturn(path,requestNumber){
	...
	document.stockReturnForm.action = path + "/jsp/stockReturnAction.do?do=viewStockReturn&action=view"+param;
	document.stockReturnForm.submit();
	return true;
}
...
function createNewStockReturn(path){
	...
	document.stockReturnForm.action = path + "/jsp/stockReturnAction.do?do=createNewStock"+param;
	document.stockReturnForm.submit();
	return true;
}
	    	...
	    	<!-- PROGRAM HEADER -->
	      	...
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		...
	      		<tr>
						...
						<table align="center" border="0" cellpadding="3" cellspacing="0" class="body" width="100%">
						 <tr>
							<td colspan="4" align="center">
							      <font color="black" size="5"> <b> ค้นหาข้อมูล ใบอนุมัติให้คืนคลัง PENS รอทำลาย</b> </font>
							 </td>
						 </tr>
						 <tr>
					...
					<!-- RESULT -->
					<!-- Paging -->
					 <c:if test="${stockReturnForm.results != null}">
						<span class="pagebanner">พบรายการ  ${stockReturnForm.totalRow} รายการ  ,แสดงรายการทั้งหมด </span>
						 <span class="pagelinks">หน้า
							<c:forEach var="i" begin="1" end="${stockReturnForm.totalPage}">
							     <c:if test="${stockReturnForm.curPage != i}">
					    ...
					    <table align="center" border="0" cellpadding="3" cellspacing="2" class="tableSearchNoWidth" width="100%">
							...
							<c:forEach var="item" items="${stockReturnForm.results}" varStatus="rows">
							<c:choose>
