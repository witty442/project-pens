...
public class MCustomer extends I_Model<Customer> {
	...
	public double getInvoiceAmount(int customerId) throws Exception {
		...
		try {
			conn = new DBCPConnectionProvider().getConnection(conn);
			String sql = "select sum(o.net_amount) net_amount ";
			sql += "from t_order o ";
			...
			sql = "select sum(r.receipt_amount) receipt_amount ";
			sql += "from t_receipt r ";
			sql += "where r.doc_status = 'SV' ";
			sql += "  and r.CUSTOMER_ID = " + customerId;
            
			//logger.debug("sql:"+sql);
			rst = stmt.executeQuery(sql);
			if (rst.next()) paidAmt = rst.getDouble("receipt_amount");
			...
			*/
			sql = "select sum(cn.total_amount) as creditNoteAmt from t_credit_note cn where cn.active = 'Y' and cn.customer_id = "+customerId;
