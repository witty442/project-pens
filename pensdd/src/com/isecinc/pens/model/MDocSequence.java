...
public class MDocSequence extends I_Model<DocSequence> {
	...
	public DocSequence[] search(Connection conn,String whereCause) throws Exception {
		List<DocSequence> pos = search(conn,TABLE_NAME, COLUMN_ID, whereCause, DocSequence.class);
		if (pos.size() == 0) return null;
		DocSequence[] array = new DocSequence[pos.size()];
		array = pos.toArray(array);
		return array;
	...
	public DocSequence[] search(String whereCause, Connection conn) throws Exception {
		conn = new DBCPConnectionProvider().getConnection(conn);
		String sql = "SELECT * FROM " + TABLE_NAME + " WHERE 1 = 1 " + (whereCause.length() > 0 ? whereCause : "");
		List<DocSequence> pos = Database.query(sql, null,  DocSequence.class, conn);
		if (pos.size() == 0) return null;
		DocSequence[] array = new DocSequence[pos.size()];
		array = pos.toArray(array);
	...
	public boolean saveDocSeq(DocSequence docSequence, int activeUserID, Connection conn) throws Exception {
		if(conn == null)
			conn = new DBCPConnectionProvider().getConnection(conn);
		
		int id = SequenceProcess.getNextValue(TABLE_NAME);
		Object[] values ={ id, docSequence.getDoctypeID() , docSequence.getSalesCode(), docSequence.getOrderType()
				,docSequence.getStartNo(), docSequence.getCurrentNext(),docSequence.getCurrentYear(), docSequence.getCurrentMonth(), docSequence.getActive()};
		return super.save(TABLE_NAME, columns, values, docSequence.getId(), conn);
	}
}
