...
public class MReceiptLine extends I_Model<ReceiptLine> {
	...
	private String[] columns = { COLUMN_ID, "LINE_NO", "RECEIPT_ID", "AR_INVOICE_NO", "SALES_ORDER_NO",
			...
			"DESCRIPTION", "ORDER_LINE_ID","TAXINVOICE_ID" /** "COMPLETE" **/
	};
	...
	public double calculateCreditAmount(Order order) throws Exception {
		...
		Statement stmt = null;
		ResultSet rst = null;
		double creditAmt = order.getNetAmount();
		double paidAmt = 0;
		try {
	...
	public List<ReceiptLine> lookUpOutstanding(int customerId) throws Exception {
		...
		ResultSet rst = null;
		try {
			String sql = "select order_id,order_no,sales_order_no,ar_invoice_no,net_amount ";
			sql += "from t_order ";
			sql += "where 1=1 ";
	...
	public List<ReceiptLine> findTaxInvoiceId(int receiptId) throws Exception {
		List<ReceiptLine> pos = new ArrayList<ReceiptLine>();
		Connection conn = null;
		Statement stmt = null;
		ResultSet rst = null;
		try {
			String sql = "select distinct taxinvoice_id from t_receipt_line ";
			sql += " where 1=1 and receipt_id ="+receiptId;
			...
			while (rst.next()) {
				rl = new ReceiptLine();
				rl.setReceiptId(receiptId);
				rl.setTaxinvoiceId(rst.getInt("taxinvoice_id"));
				pos.add(rl);
			}
