...
public class InvoicePaymentReportProcess extends I_ReportProcess<InvoicePaymentReport> {
	...
	public List<InvoicePaymentReport> searchReport(InvoicePaymentReport t, User user, Connection conn, int type)
		...
		try {
			...
			case 3: {
				...
				sql.append("\n  AND rc.RECEIPT_DATE <= '" + DateToolsUtil.convertToTimeStamp(t.getEndDate()) + "' ");
				sql.append("\n  AND rcl.ORDER_ID IN ( ");
				sql.append("\n  SELECT order_id FROM t_order od  ");
				sql.append("\n  WHERE od.DOC_STATUS = rc.DOC_STATUS ) ");
			}
	...
	{ 
		...
		reportSql = reportSql.append("SELECT inv.name as inv_name , inv.description , rcb.BANK , rcb.CHEQUE_NO, rcb.CHEQUE_DATE , us.CODE, us.NAME ")
						  	...
						  	.append(", od.ORDER_DATE, rcb.receipt_amount, rcb.payment_method, IF(od.ORDER_DATE <>rch.receipt_date ,'CREDIT','CASH' ) as payment_term ")
		...
		while(rset.next()){
			...
			inv.setChequeNo(rset.getString("CHEQUE_NO"));
			if (!inv.getChequeNo().equals("")) {
