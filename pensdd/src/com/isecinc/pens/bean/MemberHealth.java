...
public class MemberHealth extends Customer implements Serializable {
	 ...
	 * @throws Exception
	 */
	public MemberHealth(ResultSet rst) throws Exception {
		// Mandatory
		setId(rst.getInt("REFERENCE_ID"));
		...
		setCustomerType(ConvertNullUtil.convertToString(rst.getString("CUSTOMER_TYPE")).trim());
		...
		setIsvip(ConvertNullUtil.convertToString(rst.getString("ISVIP")));
		setCreditcardExpired(ConvertNullUtil.convertToString(rst.getString("CREDITCARD_EXPIRED")).trim());
		// set display label
		setDisplayLabel();
		
		setCustomerId(rst.getInt("CUSTOMER_ID")); // edit by tutiya
        setReferenceId(rst.getInt("REFERENCE_ID")); // edit by tutiya
	}
...
/*	
	CUSTOMER_ID,I,0,INTEGER,,N
	REFERENCE_ID,UI,1,INTEGER,,N
	CUSTOMER_TYPE,UI,2,VARCHAR,,N
	CODE,UI,3,VARCHAR,,N
