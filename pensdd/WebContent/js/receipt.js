...
function setValueToReceitpBy(path, payBy){
	...
	if(payBy.method=='CS'){
		...
		if (checkAmtLessthanZero(Number(payBy.recAmount).toFixed(2))){
		   tbl.rows[payBy.rows].cells[8].innerHTML = '<input type="checkbox" name="pb.writeOff" value="Y" checked>';
		}else{
		   tbl.rows[payBy.rows].cells[8].innerHTML = '<input type="checkbox" name="pb.writeOff" value="Y">';
		}
	}else{
...
function applyBill(path, rowNo, type , recAmount, seedId) {
	...
	if (rowNo == null)
		window.open(path + "/jsp/pop/applyBillPopup.jsp?selected="+selected+"&rec="+recAmount+"&type="+type+"&seed="+seedId+"&cust="+customerId+"&cns="+cns, "ApplyBill",
			"width=939,height=400,location=No,resizable=No");
	else
		window.open(path + "/jsp/pop/applyBillPopup.jsp?row=" + rowNo+"&selected="+selected+"&rec="+recAmount+"&type="+type+"&seed="+seedId+"&cust="+customerId+"&cns="+cns, "ApplyBill",
			"width=939,height=400,location=No,resizable=No");
	return;
}
