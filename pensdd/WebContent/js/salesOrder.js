...
//					tb.rows[objValue.row].cells[c].name="line.totalAmt.display";
	...
	//Payment Method
	if(paymentMethod=="CS"){
		tbl.rows[objValue.row].cells[c++].innerHTML="เงินสด";
	}
	else if(paymentMethod=="CR"){
		tbl.rows[objValue.row].cells[c++].innerHTML="บัตรเครดิต";
	}
	else if(paymentMethod=="CH"){
		tbl.rows[objValue.row].cells[c++].innerHTML= "เช็ค";
	}	
...
function createProductList(){
	...
	if(Number(totalReceiptPlanAmt).toFixed(2) != Number(totalAmt).toFixed(2)){
		var answer = confirm("ยอดเก็บเงิน ("+addCommas(totalReceiptPlanAmt)+") ไม่เท่ากับจำนวนยอดสุทธิ ("+addCommas(totalAmt)+") \nท่านยังต้องการบันทึกรายการหรือไม่  ? ");
		if(answer)
			return true;
		else{
			divlines.innerHTML = "";
