...
public class DBBackUpManager {
	...
	private  String[] processBackup(HttpServletRequest request,User user ) throws OutOfMemoryError{
		...
		String ftpFilePath = "";
		try{
			logger.debug("Start processBackup ");
	...
	private  List<List<DBBean>> listTableBackUp(String schema,Connection conn,String exceptTable) throws Exception{
		...
		try{
			...
			if( !Utils.isNull(exceptTable).equals(""))
			sql.append("  and table_name not in("+exceptTable+")");
