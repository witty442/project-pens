...
public class SequenceHelper {
	...
	public static Integer getNextValue(String tableName) throws Exception {
		...
		try {
			...
			sql.append("SELECT NextValue FROM pensbme_c_sequence ");
			...
			if (rst.next()) {
				...
				sql.delete(0, sql.length());
				sql.append("INSERT INTO pensbme_c_sequence(NAME,STARTNO,NEXTVALUE) ");
			...
			// Update Value
			sql.delete(0, sql.length());
			sql.append("UPDATE pensbme_c_sequence SET NextValue = NextValue + 1 ");
			sql.append("WHERE NAME = '" + tableName + "' ");
			logger.debug(sql.toString());
			stmt.execute(sql.toString());
			// commit
