...
public class InterfaceHelperExcel extends InterfaceUtils{
	...
	public static TableBean initImportConfig(Connection conn,String path,User userBean,String tableName) throws Exception {
		TableBean tableBean = new TableBean();
		try {
			
			tableBean.setTableName(tableName);
			tableBean.setFileFtpName("");
			tableBean.setSource("Form File");
			tableBean.setDestination("PENSBME");
			tableBean.setTransactionType("BME");
			tableBean.setAuthen("ALL");
			tableBean.setChildTable("N");
			tableBean.setActionDB("I");
			tableBean.setPreFunction("N");
			tableBean.setPostFunction("N");
			
			if("PENSBME_SALESWACOAL_FROM_LOTUS".equalsIgnoreCase(tableName)){
				tableBean.setStartRowData(4);
			}
				...
				if (!Utils.isBlank(lineStr)) { // exclude blank line
					...
					if (!cs[0].startsWith("#")) { // exclude header, comment
						try {
							...
							col.setValidateFunc(Utils.isNull(p[8]));
							columnList.add(col);
		...
		try{
			...
			for(i=0;i<tableBean.getColumnBeanList().size();i++){
	    		...
	    		if( !Utils.isNull(colBean.getValidateFunc()).equalsIgnoreCase("N")){
	    			ValidateImportHelperExcel.validate(conn, colBean, lineArray, userBean,tableBean.getTableName());
	    		}
