...
public class ReportOnhandTops_SQL {
	...
	public static StringBuilder genSQL(Connection conn,OnhandSummary c,String summaryType) throws Exception{
		...
		try {
			...
			sql.append("\n FROM(  ");
					...
					//sql.append("\n substr(P.inventory_item_desc,0,INSTRB(inventory_item_desc,' ',1,1)-1) as group_type ");
					...
					sql.append("\n FROM PENSBME_SALES_FROM_TOPS L ");
					sql.append("\n WHERE 1=1 ");
					if( !Utils.isNull(c.getSalesDate()).equals("")){
    		...
    		sql.append("\n LEFT OUTER JOIN(	 ");
					...
					sql.append("\n FROM PENSBI.PENSBME_SALES_FROM_TOPS L ");
					sql.append("\n WHERE 1=1 ");
					if( !Utils.isNull(c.getSalesDate()).equals("")){
                        sql.append("\n AND L.sales_date <= to_date('"+christSalesDateStr+"','dd/mm/yyyy')  ");
					}
