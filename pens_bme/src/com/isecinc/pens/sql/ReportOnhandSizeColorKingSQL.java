		...
		try {
			...
			sql.append("\n FROM(  ");
			   sql.append("\n SELECT DISTINCT AA.* FROM(");
					...
					if( !Utils.isNull(c.getGroup()).equals("")){
						sql.append("\n AND L.group_item IN("+SQLHelper.converToTextSqlIn(c.getGroup())+") ");
					}
					sql.append("\n UNION ");
					...
					if( !Utils.isNull(c.getGroup()).equals("")){
						sql.append("\n AND MI.pens_desc2 IN("+SQLHelper.converToTextSqlIn(c.getGroup())+") ");
					}
	      ...
	      /******* INIT MTT STOCK ***********/
		   ...
		   /********** Sale Out ***************************/
			...
			sql.append("\n GROUP BY M.customer_code,P.inventory_item_code,MP.pens_desc2, MP.material_master,MP.barcode");
			sql.append("\n )SALE_IN ");
