...
public class PickStockDAO extends PickConstants{
	...
	public static PickStock saveBox(Connection conn,PickStock h) throws Exception{
		...
		try{
			...
			if(Utils.isNull(h.getIssueReqNo()).equals("")){
				//Gen issueReqNo
				h.setIssueReqNo(genIssueReqNoCaseSaveBox(conn,tDate));
				h.setIssueReqStatus(STATUS_OPEN);
				...
				if(h.getItems() != null && h.getItems().size()>0){
				   for(int i=0;i<h.getItems().size();i++){
					   ...
					   //set 4 check update barcode head
					   boxNoMapAll.put(l.getBoxNo(), l.getBoxNo());
					   
					   //save item 
					    insertPickStockLineFromBarcodeItem(conn,l);
				...
				//delete pick Stock Item and update barcode item status='CLOSE'
				boxNoMapAll = deletePickStockItemAndUpdateBarcodeToClose(conn,h);
	...
	private static Map<String ,String> insertPickStockLineFromBarcodeItem(Connection conn,PickStock o ) throws Exception {
		...
		try {
			...
			//** Status = CLOSE ONLY  For case Pick by group part of Box*/
			...
			while(rst.next()) {
			   ...
			   // WHERE  BOX_NO = ? and  material_master =? and group_code =? and pens_item = ? and job_id = ? and line_id =?;
	...
	private static Map<String,String> deletePickStockItemAndUpdateBarcodeToClose(Connection conn,PickStock h) throws Exception{
		int no = 1;
		Map<String,String> boxNoMapAll = new HashMap<String, String>();
		try{
			  List<PickStock> pickStockItemList = searchPickStockItemListByIssueReqNo(conn, h.getIssueReqNo());
			  logger.debug("pickStockItemList:"+pickStockItemList.size());
	...
	public static PickStock searchPickStockItemByIssueReqNo4Report(Connection conn,PickStock o ) throws Exception {
		...
		try {
			...
			sql.append("\n ,(select h.invoice_no from PENSBI.PENSBME_PICK_STOCK h ");
			sql.append("\n   where h.issue_req_no= '"+Utils.isNull(o.getIssueReqNo())+"' )as invoice_no ");

			sql.append("\n ,(select max(h.barcode) from PENSBI.PENSBME_ONHAND_BME_LOCKED h ");
			sql.append("\n   where h.MATERIAL_MASTER=i.MATERIAL_MASTER");
			sql.append("\n   and h.group_item =i.group_code ");
			sql.append("\n   and h.pens_item =i.pens_item ");
			sql.append("\n ) as barcode ");
	...
	// ( Running :  TYYMMxxxx  เช่น T57040001 เป็นต้น	
	 ...
	 //Start S
	 private static String genIssueReqNoCaseSaveBox(Connection conn,Date tDate) throws Exception{
		   String orderNo = "";
		   try{
			   String today = df.format(tDate);
			   String[] d1 = today.split("/");
			   ...
			   orderNo = "S"+new DecimalFormat("00").format(curYear)+new DecimalFormat("00").format(curMonth)+new DecimalFormat("0000").format(seq);
		   }catch(Exception e){
			   throw e;
		   }
		  return orderNo;
	...
	public static void updatePickStockHeadModel(Connection conn,PickStock o) throws Exception{
		...
		try{
			...
			sql.append(" ,total_box =? ,total_issue_qty = ? ,delivery_date = ? \n ");
			if( !Utils.isNull(o.getConfirmIssueDate()).equals("")){
...
//PK  (ISSUE_REQ_NO,JOB_ID,BOX_NO,LINE_ID)	
	...
	public static Map<String,String> updateBarcodeToCloseFromStockPickItem(Connection conn,PickStock h) throws Exception{
		...
		int no = 1;
		try{
		    List<PickStock> pickStockItemByGroupList = searchPickStockItemListByIssueReqNo(conn, h.getIssueReqNo());
	...
	public static List<PickStock> searchPickStockItemListByIssueReqNo(Connection conn,String issueReqNo) throws Exception {
		PreparedStatement ps = null;
		ResultSet rst = null;
		StringBuilder sql = new StringBuilder();
		PickStock h = null;
