...
public class OrderFridayDAO {
	protected static Logger logger = Logger.getLogger("PENS");
	public int MAX_ORDER_SPLIT_ORDER_NO = 450;//default 450
	
	
	public int getTotalRowBMEItemFriday(Connection conn,Order o ) throws Exception {
		Statement stmt = null;
		ResultSet rst = null;
        int totalRow = 0;
		StringBuilder sql = new StringBuilder();
		try {
			sql.append("\n SELECT count(*) as total_row  from PENSBME_ONHAND_BME_FRIDAY    \n");
	...
	public List<Order> prepareNewOrderFriday(Connection conn,Order o,List<StoreBean> storeList,User user) throws Exception {
		Statement stmt = null;
		ResultSet rst = null;
		...
		try {
			...
			sql.append("\n  SELECT *  from PENSBME_ONHAND_BME_FRIDAY  ");
	...
	public List<Order> prepareNewOrderFriday(Connection conn,Order o,List<StoreBean> storeList,User user,int pageNumber,int pageSize) throws Exception {
		...
		try {
			...
			sql.append("\n  SELECT s.* FROM(  \n");
				...
				sql.append("\n   where m.reference_code ='"+Constants.STORE_TYPE_FRIDAY_ITEM+"' and m.interface_desc = h.barcode) as item_oracle ");
				sql.append("\n ,(SELECT max(m.interface_value) from PENSBME_MST_REFERENCE m   ");
				sql.append("\n   where m.reference_code ='"+Constants.STORE_TYPE_FRIDAY_ITEM+"' and m.interface_desc = h.barcode) as item_style ");
				sql.append("\n from PENSBME_ONHAND_BME_FRIDAY h   \n");
				sql.append("\n where 1=1 AND onhand_qty <> 0 and status <> 'ERROR'  ");
			...
			//Get StoreBeanMap 
			Map<String, StoreBean> storeBeanOrderMap = getStoreBeanOrderMapFriday(conn, o.getStoreType(), orderDate);
	...
	public static Map<String,String> initStoreTypeFridayMap() throws Exception{
		...
		try{
			...
			sql.append(" SELECT * FROM PENSBI.PENSBME_MST_REFERENCE WHERE reference_code ='Customer' and pens_value ='"+Constants.STORE_TYPE_FRIDAY_CODE+"'  \n");
			ps = conn.prepareStatement(sql.toString());
			rs = ps.executeQuery();
