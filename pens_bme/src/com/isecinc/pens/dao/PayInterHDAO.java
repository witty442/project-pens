...
import com.pens.util.DollarText;
import com.pens.util.Utils;
import com.pens.util.seq.SequenceProcess;

public class PayInterHDAO {
		...
		public static int searchTotalHead(Connection conn,PayBean o ) throws Exception {
			...
			try {
				sql.append("\n select count(*) as c from(");
				   ...
				   sql.append("\n from doc_inter_h_tran h where 1=1 ");
			...
			try {
				...
				while(rst.next()) {
				   ...
				   h.setDR_DESC(Utils.isNull(rst.getString("DR_DESC")));
					...
					if( Utils.isNull(h.getPaymethod()).equals("TR")){
						h.setTrFlag("on");
					}
	               ...
	               if(getTrans){
	            	   ...
	            	   //convert to dollar text
	            	   String[] aa = Utils.isNull(h.getTotalAmount()).split("\\.");
		   			   if(aa.length > 1){
		   				  String d1 = DollarText.convert(new Double(aa[0]).longValue());		
		   				  String d2 = DollarText.convert(new Double(aa[1]).longValue());	
		   				  h.setTotalAmountLetter(d1+" US DOLLAR " +d2+" CENT");
		   			   }else{
		   				  h.setTotalAmountLetter(DollarText.convert(new Double(h.getTotalAmountDouble()).longValue())+ " US DOLLAR");	
		   			   }
	...
	public static PayBean searchTranDetailList(Connection conn,PayBean o) throws Exception {
			...
			try {
			   sql.append("\n select * from DOC_INTER_H_TRAN_DETAIL");
			   sql.append("\n where 1=1 ");
	...
	public static void insertDocTran(Connection conn,PayBean o) throws Exception{
		...
		try{
			...
			sql.append(" INSERT INTO PENSBI.doc_inter_h_tran \n");
 ...
 public static void insertDocTranDetail(Connection conn,PayBean o) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" INSERT INTO PENSBI.DOC_INTER_H_TRAN_DETAIL \n");
	...
	public static int updateDocTran(Connection conn,PayBean o) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" UPDATE PENSBI.doc_inter_h_tran SET DOC_DATE =? ,Pay_to_name = ? ,DEPT_ID=?,SECTION_ID =?,PAY_METHOD =?,  \n");
	...
	public static int updateDocTranDetail(Connection conn,PayBean o) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" UPDATE DOC_INTER_H_TRAN_DETAIL SET Account_name = ? ,Description=?, Amount=? \n");
			sql.append(" ,UPDATE_USER =? ,UPDATE_DATE = ?   \n");
	 ...
	 public static String genDocNo(Date date) throws Exception{
		   ...
		   try{
			 ...
			 //get Seq
			   int seq = SequenceProcess.getNextValue(conn,"doc_inter_h_tran");
