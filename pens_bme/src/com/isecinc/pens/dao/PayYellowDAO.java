...
public class PayYellowDAO {
		...
		public static int searchTotalHead(Connection conn,PayBean o ) throws Exception {
			...
			try {
				sql.append("\n select count(*) as c from(");
				   ...
				   sql.append("\n from doc_yellow_tran h where 1=1 ");
					...
					if( !Utils.isNull(o.getDeptId()).equals("")){
						sql.append("\n and h.dept_id = '"+Utils.isNull(o.getDeptId())+"'");
					}
					if( !Utils.isNull(o.getSectionId()).equals("")){
						sql.append("\n and h.section_id = '"+Utils.isNull(o.getSectionId())+"'");
					}
		...
		public static PayBean searchHeadModel(Connection conn,PayBean o ,boolean getTrans,boolean allRec ,int currPage,int pageSize) throws Exception {
			...
			try {
				...
				while(rst.next()) {
				   ...
				   h.setEmployeeName(Utils.isNull(rst.getString("employee_name")));
	...
	public static PayBean searchTranDetailList(Connection conn,PayBean o) throws Exception {
			...
			try {
			   sql.append("\n select * from doc_yellow_tran_detail");
			   sql.append("\n where 1=1 ");
	...
	public static void insertDocTran(Connection conn,PayBean o) throws Exception{
		...
		try{
			...
			sql.append(" INSERT INTO PENSBI.DOC_YELLOW_TRAN \n");
			sql.append(" (DOC_NO, DOC_DATE, EMPLOYEE_NAME, DEPT_ID, SECTION_ID," );
			...
			ps.setString(c++, o.getEmployeeName());
 ...
 public static void insertDocTranDetail(Connection conn,PayBean o) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" INSERT INTO PENSBI.DOC_YELLOW_TRAN_DETAIL \n");
	...
	public static int updateDocTran(Connection conn,PayBean o) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" UPDATE PENSBI.DOC_YELLOW_TRAN SET DOC_DATE =? ,employee_name = ? ,DEPT_ID=?,SECTION_ID =? \n");
			...
			ps.setString(c++, Utils.isNull(o.getEmployeeName()));
	...
	public static int updateDocTranDetail(Connection conn,PayBean o) throws Exception{
		...
		try{
			StringBuffer sql = new StringBuffer("");
			sql.append(" UPDATE DOC_YELLOW_TRAN_DETAIL SET Account_name = ? ,Description=?, Amount=? \n");
			sql.append(" ,UPDATE_USER =? ,UPDATE_DATE = ?   \n");
	 ...
	 public static String genDocNo(Date date) throws Exception{
		   ...
		   try{
			 ...
			 //get Seq
			   int seq = SequenceProcess.getNextValue(conn,"DOC_YELLOW_TRAN");
