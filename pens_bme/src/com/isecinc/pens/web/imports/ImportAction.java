...
public class ImportAction extends I_Action {
	...
	protected String prepare(String id, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			importForm.setPageTypeImport(Utils.isNull(request.getParameter("page")));
	...
	public ActionForward importExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
        ...
        if("lotus".equalsIgnoreCase(Utils.isNull(request.getParameter("page")))){
            ...
            return importMaster(mapping, importForm, request, response);
            
        }else if("physical".equalsIgnoreCase(Utils.isNull(request.getParameter("page")))){
        	
            return importPhysical(mapping, importForm, request, response);  
            
        }else if("return_wacoal".equalsIgnoreCase(Utils.isNull(request.getParameter("page")))){
        	
            return importReturnWacoal(mapping, importForm, request, response);    
        }else{
        	
        	return importFromWacoal(mapping, importForm, request, response);
	...
	public ActionForward importFromWacoal(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		try {
			...
			java.util.Date fileNameAsOfDate = Utils.parse(fileName.substring(9,17),Utils.YYYY_MM_DD_WITHOUT_SLASH,Utils.local_th);
			...
			if( !Utils.isNull(lastFileNameImport).equals("")){
				java.util.Date lastFileNameAsOfDate = Utils.parse(lastFileNameImport.substring(9,17),Utils.YYYY_MM_DD_WITHOUT_SLASH,Utils.local_th);
		         ...
		         if(!Utils.isNull(lines).equals("")){
		        	try{
				        ...
				        // logger.debug("item["+item+"]length["+item.length()+"]");
				         ...
				         oh.setMaterialMaster(Utils.isNull(materialMaster));
				         //Find pens_item **/
				         pensItem = importDAO.getItemByBarcode(conn, oh.getBarcode());
