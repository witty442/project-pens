 ...
 * 
 */
public class OrderFridayAction extends I_Action {
		...
		try {
			...
			// if( !userLastAccessPath.equals("")){
			 ...
			 if("new".equalsIgnoreCase(request.getParameter("action"))){
				 ...
				 List<References> storeTypeList = importDAO.getStoreTypeFridayList(conn);
	...
	protected String search(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		User user = (User) request.getSession().getAttribute("user");
		OrderFridayDAO orderDAO = new OrderFridayDAO();
		Connection conn = null;
		int totalRow = 0;
		int totalPage = 0;
		int pageNumber = 1;
		...
		try {
		        		...
		        		for(int row=0;row<groupArr.length;row++ ){
	                        ...
	                        if("".equals(orderNoLine)){
	                        	...
	                        	if( !"".equals(o.getQty()) && !"0".equals(o.getQty())){
		                        	...
		                        	OrderFridayDAO.saveOrder(conn, o);
	                        	}
	                        }else{
	                        	//update 
	                        	...
	                        	if("0".equals(o.getQty())){
	                        	   OrderFridayDAO.deleteOrder(conn, o);
	                        	}else{
	                        	   OrderFridayDAO.updateOrder(conn, o);
	                        	}
			...
			List<Order> results = new OrderFridayDAO().prepareNewOrderFriday(conn,orderForm.getOrder(),storeList, user,pageNumber,pageSize);
...
*/			
			if(haveError == false){
				...
				request.setAttribute("Message", "บันทึกข้อมูลเรียบร้อยแล้ว");
				 //Research 
			    //List<Order> results = new OrderFridayDAO().prepareNewOrder(conn,orderForm.getOrder(),storeList, user);
		        //request.getSession().setAttribute("results", results);
		       // request.getSession().setAttribute("itemErrorMap", null);
				...
				request.setAttribute("Message", "ไม่สามารถบันทึกข้อมูลได้");
				 //Research 
			    List<Order> results = new OrderFridayDAO().prepareNewOrderFriday(conn,orderForm.getOrder(),storeList, user);
		        request.getSession().setAttribute("results", results);
