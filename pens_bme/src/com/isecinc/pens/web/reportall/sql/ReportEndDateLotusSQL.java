...
public class ReportEndDateLotusSQL {
	private static Logger logger = Logger.getLogger("PENS");
	 ...
	 public static StringBuilder genSQL(Connection conn,ReportAllBean c,User user,String summaryType,String typeGen) throws Exception{
			...
			sql.append("\n SELECT A.* ");
			sql.append("\n,(SELECT NVL(MAX(RETAIL_PRICE_BF),0) FROM PENSBI.PENSBME_ONHAND_BME_LOCKED T "
		 ...
		 sql.append("\n  )MM  ");
		 sql.append("\n  WHERE MM.GROUP_TYPE NOT IN(select group_code from PENSBI.pensbme_group_unuse_lotus un)");
         sql.append("\n )M ");
         sql.append("\n LEFT OUTER JOIN(	 ");
         sql.append("\n /******************* BEGINING ****************************************/ ");
					...
					//NOT IN LOTUS UNUSE
					sql.append("\n\t AND L.group_code NOT IN (select group_code from PENSBI.pensbme_group_unuse_lotus un )");
 		 ...
 		 sql.append("\n/******************* SALE OUT ****************************************/");
				...
				sql.append(genWhereCondDate(control,"L.sales_date"));
				//NOT IN LOTUS UNUSE
				sql.append("\n\t AND L.pens_group_type  NOT IN (select group_code from PENSBI.pensbme_group_unuse_lotus un )");
				...
				sql.append("\n /******************* SALE RETURN ****************************************/");
					...
					sql.append(genWhereCondDate(control,"J.close_date"));
					//NOT IN LOTUS UNUSE
					sql.append("\n\t AND I.group_code NOT IN (select group_code from PENSBI.pensbme_group_unuse_lotus un )");
				...
				sql.append("\n /******************* STOCK_ISSUE ****************************************/");
				 	sql.append("\n\t SELECT ");
				 	sql.append("\n\t L.store_code as customer_code,L.item_issue as pens_item ");
					...
					sql.append("\n\t AND L.STORE_CODE LIKE '020047-%'");
				 	...
				 	//NOT IN LOTUS UNUSE
					sql.append("\n\t AND L.item_issue_desc NOT IN (select group_code from PENSBI.pensbme_group_unuse_lotus un )");
				...
				sql.append("\n /******************* STOCK RECEIPT ****************************************/");
				  sql.append("\n\t SELECT ");
				  sql.append("\n\t L.STORE_CODE as customer_code,L.item_receipt as pens_item ");
				  ...
				  //NOT IN LOTUS UNUSE
				  sql.append("\n\t AND L.item_receipt_desc NOT IN (select group_code from PENSBI.pensbme_group_unuse_lotus un )");
				...
				sql.append("\n /******************* STOCK_SHORT ****************************************/");
				  ...
				  sql.append(genWhereCondDate(control,"L.transaction_date"));
				  //NOT IN LOTUS UNUSE
				  sql.append("\n\t AND L.item_adjust_desc NOT IN (select group_code from PENSBI.pensbme_group_unuse_lotus un )");
				...
				sql.append("\n )STOCK_SHORT ");
				sql.append("\n ON  M.customer_code = STOCK_SHORT.customer_code ");
				sql.append("\n AND M.pens_item = STOCK_SHORT.pens_item ");	 
