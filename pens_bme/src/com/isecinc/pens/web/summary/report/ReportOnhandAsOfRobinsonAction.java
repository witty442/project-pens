...
public class ReportOnhandAsOfRobinsonAction {
	...
	try{
		//set Page Name
		summaryForm.setPage("onhandAsOfRobinson");
		//Init Connection
		...
		if( !Utils.isNull(storeCodeCheck).equals("ALL") && storeCodeCheckArr.length==1){
			...
			if(pass){
				...
				OnhandSummary summary = searchOnhandAsOfRobinson(conn,summaryForm.getOnhandSummary(),initDate,user,summaryForm.getSummaryType());
			...
			if(storeList != null && storeList.size() >0){
				...
				for(int i=0;i<storeList.size();i++){
					...
					sql = ReportOnhandAsOfRobinsonSQL.genSQL(conn, c, user, initDate,summaryForm.getSummaryType());
	...
	public static OnhandSummary searchOnhandAsOfRobinson(Connection conn,OnhandSummary c,Date initDate,User user,String summaryType) throws Exception{
		   Statement stmt = null;
			ResultSet rst = null;
			...
			try {
				sql = ReportOnhandAsOfRobinsonSQL.genSQL(conn, c, user, initDate,summaryType);
				stmt = conn.createStatement();
				rst = stmt.executeQuery(sql.toString());
				while (rst.next()) {
