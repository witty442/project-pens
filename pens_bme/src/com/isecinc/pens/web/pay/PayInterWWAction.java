...
import com.isecinc.pens.dao.PayInterWWDAO;
 ...
 * 
 */
public class PayInterWWAction extends I_Action {
	...
	public ActionForward prepare2(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		PayInterWWForm aForm = (PayInterWWForm) form;
		...
		try {
			...
			if("new".equals(action)){
				...
				billTypeList.addAll(PayInterWWDAO.searchDeptList(new PopupForm(),""));
				...
				cri.setDocNo("");
				aForm.setBean(PayInterWWDAO.searchHead(conn,cri,false,false,1,pageSize));
	...
	public ActionForward search2(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		try {
			...
			if("newsearch".equalsIgnoreCase(action) || "back".equalsIgnoreCase(action)){
				...
				//get Total Record
				aForm.setTotalRecord(PayInterWWDAO.searchTotalHead(conn,aForm.getBean()));
				//calc TotalPage
				...
				PayBean paybean = PayInterWWDAO.searchHead(conn,aForm.getBean(),false,allRec,currPage,pageSize);
	...
	protected String prepare(ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if("copy".equalsIgnoreCase(mode)){
				...
				PayBean bean = PayInterWWDAO.searchHead(c,true,false,1,pageSize).getItems().get(0);
	...
	protected String prepare(String id, ActionForm form, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		PayInterWWForm summaryForm = (PayInterWWForm) form;
		try {
	...
	protected String search(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		PayInterWWForm orderForm = (PayInterWWForm) form;
		User user = (User) request.getSession().getAttribute("user");
		String msg = "";
		try {
	...
	protected String save(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if( !Utils.isNull(h.getDocNo()).equals("")){
				int update = PayInterWWDAO.updateDocTran(conn, h);
				if(update==0){
				   PayInterWWDAO.insertDocTran(conn, h);
				}
			}else{
				//Get DocNo
				h.setDocNo(PayInterWWDAO.genDocNo(new Date()));
				PayInterWWDAO.insertDocTran(conn, h);
			}
			...
			for(int i=0;i<4;i++){
				...
				if(item.getLineId()==0){
					//insert
					item.setLineId(i+1);
					PayInterWWDAO.insertDocTranDetail(conn, item);
				}else{
					//update
					PayInterWWDAO.updateDocTranDetail(conn, item);
				}
				
			}
			
			//Search Again
			PayBean bean = PayInterWWDAO.searchHead(conn,h,true,false,1,pageSize).getItems().get(0);
		    aForm.setBean(bean);
	...
	public ActionForward printReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response) {
		...
		PayInterWWForm reportForm = (PayInterWWForm) form;
		User user = (User) request.getSession().getAttribute("user");
		...
		try {
			...
			PayBean h = PayInterWWDAO.searchHead(conn,cri,true,false,1,pageSize).getItems().get(0);
			logger.debug("result:"+h.getDocNo());
			
			if(h != null){
				//Head
				parameterMap.put("companyName", "บริษัท เพนส์ เวิลด์ไวด์ เทรดดิ้ง จำกัด");
