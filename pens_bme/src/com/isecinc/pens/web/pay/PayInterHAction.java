...
import com.isecinc.pens.dao.PayInterHDAO;
 ...
 * 
 */
public class PayInterHAction extends I_Action {
	...
	public ActionForward prepare2(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		PayInterHForm aForm = (PayInterHForm) form;
		...
		try {
			...
			if("new".equals(action)){
				...
				billTypeList.addAll(PayInterHDAO.searchDeptList(new PopupForm(),""));
				...
				cri.setDocNo("");
				aForm.setBean(PayInterHDAO.searchHead(conn,cri,false,false,1,pageSize));
	...
	public ActionForward search2(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		try {
			...
			if("newsearch".equalsIgnoreCase(action) || "back".equalsIgnoreCase(action)){
				...
				//get Total Record
				aForm.setTotalRecord(PayInterHDAO.searchTotalHead(conn,aForm.getBean()));
				//calc TotalPage
				...
				PayBean paybean = PayInterHDAO.searchHead(conn,aForm.getBean(),false,allRec,currPage,pageSize);
	...
	protected String prepare(ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if("copy".equalsIgnoreCase(mode)){
				...
				PayBean bean = PayInterHDAO.searchHead(c,true,false,1,pageSize).getItems().get(0);
	...
	protected String prepare(String id, ActionForm form, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		PayInterHForm summaryForm = (PayInterHForm) form;
		try {
	...
	protected String search(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		PayInterHForm orderForm = (PayInterHForm) form;
		User user = (User) request.getSession().getAttribute("user");
		String msg = "";
		try {
	...
	protected String save(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if( !Utils.isNull(h.getDocNo()).equals("")){
				int update = PayInterHDAO.updateDocTran(conn, h);
				if(update==0){
				   PayInterHDAO.insertDocTran(conn, h);
				}
			}else{
				//Get DocNo
				h.setDocNo(PayInterHDAO.genDocNo(new Date()));
				PayInterHDAO.insertDocTran(conn, h);
			}
			...
			for(int i=0;i<4;i++){
				...
				if(item.getLineId()==0){
					//insert
					item.setLineId(i+1);
					PayInterHDAO.insertDocTranDetail(conn, item);
				}else{
					//update
					PayInterHDAO.updateDocTranDetail(conn, item);
				}
				
			}
			
			//Search Again
			PayBean bean = PayInterHDAO.searchHead(conn,h,true,false,1,pageSize).getItems().get(0);
		    aForm.setBean(bean);
	...
	public ActionForward printReport(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response) {
		...
		PayInterHForm reportForm = (PayInterHForm) form;
		User user = (User) request.getSession().getAttribute("user");
		...
		try {
			...
			PayBean h = PayInterHDAO.searchHead(conn,cri,true,false,1,pageSize).getItems().get(0);
			logger.debug("result:"+h.getDocNo());
			
			if(h != null){
				//Head
				parameterMap.put("companyName", "Payment Voucher");
