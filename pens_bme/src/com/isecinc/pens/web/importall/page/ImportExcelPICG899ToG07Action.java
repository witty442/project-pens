...
public class ImportExcelPICG899ToG07Action {
	...
	public String prepare(ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			 if("new".equalsIgnoreCase(request.getParameter("action"))){
				 ...
				 ImportExcelInvBillTBean  importExcelInvBillTBean = new ImportExcelInvBillTBean();
				 pageBean.setImportExcelInvBillTBean(importExcelInvBillTBean);
				 importAllForm.setPageBean(pageBean);
			 }
		} catch (Exception e) {
	...
	public ActionForward importExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		try{
			...
			batchParaMap.put(ImportExcelPICG899ToG07Task.STORE_CODE_P, aForm.getPageBean().getImportExcelInvBillTBean().getStoreCode());
			batchParaMap.put(ImportExcelPICG899ToG07Task.SUB_INV_P, aForm.getPageBean().getImportExcelInvBillTBean().getSubInv());
			request.getSession().setAttribute(BatchTaskConstants.BATCH_PARAM_MAP,batchParaMap);
			request.getSession().setAttribute(BatchTaskConstants.DATA_FILE,aForm.getPageBean().getImportExcelInvBillTBean().getDataFile());
	...
	public ActionForward searchBatchForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response)  throws Exception {
		...
		try {
			 ...
			 BatchTaskForm batchTaskForm = new BatchTaskDAO().searchBatchLastRun(user, BatchTaskConstants.IMPORT_EXCEL_PIC_G899_TO_G07);
