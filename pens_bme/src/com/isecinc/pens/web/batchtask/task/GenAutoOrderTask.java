...
public class GenAutoOrderTask extends BatchTask implements BatchTaskInterface{
	...
	public String getDescription(){
		String desc = "Gen Auto Order <br/>";
		return desc;
	}
	public String getDevInfo(){
		return "PENSBI.PENSBME_TEMP_ONHAND_REP,PENSBI.PENSBME_CONFIG_REP,PENSBI.PENSBME_ORDER_REP";
	}
	...
	public  MonitorBean run(MonitorBean monitorModel){
		...
		try{
			...
			String storeCode = monitorModel.getBatchParamMap().get("storeCode");
			String orderDate = monitorModel.getBatchParamMap().get("orderDate");
			boolean isStoreGenerated = isStoreGenerated(connMonitor, storeCode,orderDate);
			logger.debug("isStoreGenerated["+storeCode+"]orderDate["+orderDate+"]:"+isStoreGenerated);
			if(isStoreGenerated){
				modelItem.setStatus(Constants.STATUS_FAIL);
				modelItem.setErrorMsg("ร้านค้านี้มีการ Generate ไปแล้ว");
				modelItem.setErrorCode("");
				
			}
			boolean isStoreGenStockOnhand = isStoreGenStockOnhand(connMonitor, storeCode);
			logger.debug("isStoreGenStockOnhand["+storeCode+"]:"+isStoreGenStockOnhand);
			if( !isStoreGenStockOnhand){
				modelItem.setStatus(Constants.STATUS_FAIL);
				modelItem.setErrorMsg("ร้านค้านี้ ยังไม่ Generate Stock Onhand");
				modelItem.setErrorCode("");
			}
			
			/** Start process **/ 
			if(isStoreGenerated==false && isStoreGenStockOnhand==true){
	...
	public static MonitorItemBean process(Connection connMonitor ,Connection conn,MonitorBean monitorModel ,MonitorItemBean monitorItemBean) {
	...
	public static boolean isStoreGenerated(Connection conn,String storeCode,String orderDate) throws Exception {
		PreparedStatement ps = null;
		ResultSet rst = null;
		StringBuilder sql = new StringBuilder();
		boolean isExist = false;
		try {
			String dateStr = DateUtil.stringValue(DateUtil.parse(orderDate, DateUtil.DD_MM_YYYY_WITH_SLASH,DateUtil.local_th), DateUtil.DD_MM_YYYY_WITH_SLASH);
			
			sql.append(" select count(*) as c from PENSBI.PENSBME_ORDER_REP \n");
	...
	public static boolean isStoreGenStockOnhand(Connection conn,String storeCode) throws Exception {
		PreparedStatement ps = null;
		ResultSet rst = null;
		StringBuilder sql = new StringBuilder();
		boolean isGen = false;
