...
public class GenReportCompareEndDateLotusTask extends BatchTask implements BatchTaskInterface{
	...
	public String getDescription(){
		String desc = "Gen Report Compare End Date Lotus <br/>";
		return desc;
	}
	public String getDevInfo(){
		return "PENSBI.PENSBME_ENDDATE_STOCK_TEMP";
	}
	...
	public  MonitorBean run(MonitorBean monitorModel){
		...
		try{
			...
			monitorModel.setThName("Gen Data เปรียบเทียบนับสต็อก");
	...
	public static MonitorItemBean process(Connection connMonitor ,Connection conn
		...
		ReportAllBean cri = null;
		try{
		    //get parameter
			String asOfDate = monitorModel.getBatchParamMap().get(PARAM_ASOF_DATE);
			Date asOfDateD = DateUtil.parse(asOfDate, DateUtil.DD_MM_YYYY_WITH_SLASH,DateUtil.local_th);
			String storeCode = monitorModel.getBatchParamMap().get(PARAM_STORE_CODE);
			logger.debug("asOfDate:"+asOfDate);
			
			if( !Utils.isNull(storeCode).equals("ALL")){
				//Get By Store
				storeList =  new ArrayList<StoreBean>();
				StoreBean storeBean = new StoreBean();
				storeBean.setStoreCode(storeCode);
			...
			//prepare parameter
			cri = new ReportAllBean();
			cri.setSalesDate(asOfDate);
			
			if(storeList != null && storeList.size() >0){
				for(int i=0;i<storeList.size();i++){
					StoreBean store = storeList.get(i);
					cri.setPensCustCodeFrom((store.getStoreCode()));
					
					//delete old month by yearMonth
					String sqlDel = "delete from PENSBI.PENSBME_ENDDATE_STOCK_TEMP where store_code ='"+cri.getPensCustCodeFrom()+"'";
					...
					//despicated
					//sql = ReportEndDateLotusSQL.genSQL(conn, cri, monitorModel.getUser(), "GroupCode","GenReportCompareEndDateLotus");
					
					sql = ReportEndDateLotusSQL.genSQL(conn, cri, monitorModel.getUser(), "GroupCode","GenReportCompareEndDateLotus");
					
					//sql insert 
					String sqlIns = "INSERT INTO PENSBI.PENSBME_ENDDATE_STOCK_TEMP" +
					...
					while(rst.next()){
						...
						psIns.setDate(2,new java.sql.Date(asOfDateD.getTime()));
						...
						psIns.setString(10, monitorModel.getUser().getUserName());
			...
			if(psDel != null){psDel.close();psDel =null;}
		}
		...
		try {
  		...
  		sql.append("\n LEFT OUTER JOIN(	 ");
			...
			sql.append("\n LEFT OUTER JOIN( ");
					...
					sql.append("\n NVL(SUM( ");
					sql.append("\n       CASE WHEN P.inventory_item_code ='481623' THEN NVL(INVOICED_QTY,0)*120");
					sql.append("\n            ELSE INVOICED_QTY END");
					sql.append("\n ) ,0)  as SALE_IN_QTY ");
			...
			sql.append("\n LEFT OUTER JOIN ( ");
				...
				/** special case some product Convert CTN to EA ,481623(mask)**/
				sql.append("\n NVL(SUM( ");
				sql.append("\n       CASE WHEN A.pens_item ='481623' THEN NVL(A.SALE_RETURN_QTY,0)*120");
				sql.append("\n            ELSE A.SALE_RETURN_QTY END");
