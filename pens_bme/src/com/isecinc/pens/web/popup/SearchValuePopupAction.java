	...
	protected String search(ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (currCondNo.equalsIgnoreCase("2") || currCondNo.equalsIgnoreCase("3") || currCondNo.equalsIgnoreCase("4")){
			    ...
			    request.getSession().setAttribute("VALUE_LIST", ReportProcess.getInstance().getConditionValueListByParent(currCondType,forms.getSalesBean().getCode(),forms.getSalesBean().getDesc(),filterBean));
			}else{
				request.getSession().setAttribute("VALUE_LIST", ReportProcess.getInstance().getConditionValueList(currCondType,forms.getSalesBean().getCode(),forms.getSalesBean().getDesc()));	
			}
