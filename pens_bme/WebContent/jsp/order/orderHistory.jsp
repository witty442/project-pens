...
function search(path){
	...
	form.action = path + "/jsp/orderAction.do?do=searchHistory&action=newsearch";
	form.submit();
	return true;
}
...
function gotoPage(path,pageNumber){
	...
	//if(confirm("ข้อมูลในหน้านี้ จะถูกบันทึก กรุณากดปุ่มเพื่อยืนยันการบันทึก และไปหน้าถัดไป ")){
		...
		form.action = path + "/jsp/orderAction.do?do=searchHistory&pageNumber="+pageNumber;
		form.submit();
		return true;
	//}
	//return false;
...
function exportToTextAll(path){
	var form = document.orderForm;
	form.action = path + "/jsp/orderAction.do?do=exportToTextAllHistory";
	form.submit();
	return true;
}
...
function exportSummaryToExcel(path){
	var form = document.orderForm;
	form.action = path + "/jsp/orderAction.do?do=exportSummaryToExcelHistory";
	form.submit();
	return true;
}
...
function exportDetailToExcel(path){
	var form = document.orderForm;
	form.action = path + "/jsp/orderAction.do?do=exportDetailToExcelHistory";
	form.submit();
	return true;
}
...
function clearForm(path){
	var form = document.orderForm;
	form.action = path + "/jsp/orderAction.do?do=prepareHistory&action=new";
	form.submit();
	return true;
}
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
	    	...
	    	<!-- PROGRAM HEADER -->
	      	...
	      	<jsp:include page="../program.jsp">
				<jsp:param name="function" value="OrderInquiryHistory"/>
			</jsp:include>
	      	<!-- TABLE BODY -->
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		<tr style="height: 9px;">
						     ...
						     <table align="left" border="0" cellpadding="3" cellspacing="0" >
						       <tr>
								...
								<tr>
									...
									<td>
									     รหัสร้านค้า
									    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<html:text property="order.pensCustCodeFrom" styleId="pensCustCodeFrom" size="20" onkeypress="getCustNameKeypress(event,this,'pensCustNameFrom')"/>-
									    <input type="button" name="x1" value="..." onclick="openPopupCustomer('${pageContext.request.contextPath}','from','')"/>
									    <html:text property="order.pensCustNameFrom" styleId="pensCustNameFrom" readonly="true" styleClass="disableText" size="40"/>
									</td>
								</tr>
								<tr>
									<td>
