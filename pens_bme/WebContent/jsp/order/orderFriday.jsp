...
function search(path){
	var form = document.orderFridayForm;
	var orderDate =$('#orderDate').val();
	if(orderDate ==""){
		alert("กรุณากรอกวันที่ Order");
		return false;
	}
	form.action = path + "/jsp/orderFridayAction.do?do=search&action=newsearch";
	form.submit();
	return true;
}
...
function save(path){
	...
	form.action = path + "/jsp/orderFridayAction.do?do=save";
	form.submit();
	return true;
}
...
function gotoPage(path,pageNumber){
	...
	//if(confirm("ข้อมูลในหน้านี้ จะถูกบันทึก กรุณากดปุ่มเพื่อยืนยันการบันทึก และไปหน้าถัดไป ")){
		...
		form.action = path + "/jsp/orderFridayAction.do?do=search&action=save&pageNumber="+pageNumber;
		form.submit();
		return true;
	//}
	//return false;
...
function exportToText(path){
	var form = document.orderFridayForm;
	form.action = path + "/jsp/orderFridayAction.do?do=exportToText";
	form.submit();
	return true;
}
...
function exportToTextAll(path){
	var form = document.orderFridayForm;
	form.action = path + "/jsp/orderFridayAction.do?do=exportToTextAll";
	form.submit();
	return true;
}
...
function exportToExcel(path){
	var form = document.orderFridayForm;
	form.action = path + "/jsp/orderFridayAction.do?do=exportToExcel";
	form.submit();
	return true;
}
...
function exportSummaryToExcel(path){
	var form = document.orderFridayForm;
	form.action = path + "/jsp/orderFridayAction.do?do=exportSummaryToExcel";
	form.submit();
	return true;
}
...
function exportDetailToExcel(path){
	var form = document.orderFridayForm;
	form.action = path + "/jsp/orderFridayAction.do?do=exportDetailToExcel";
	form.submit();
	return true;
}
...
function clearForm(path){
	var form = document.orderFridayForm;
	form.action = path + "/jsp/orderFridayAction.do?do=prepare&action=new";
	form.submit();
	return true;
}
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
	    	...
	    	<!-- PROGRAM HEADER -->
	      	...
	      	<jsp:include page="../program.jsp">
				<jsp:param name="function" value="OrderFriday"/>
			</jsp:include>
	      	<!-- TABLE BODY -->
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		<tr style="height: 9px;">
	      		...
	      		<tr>
		            ...
		            <td bgcolor="#f8f8f8">
						...
						<!-- BODY -->
						<html:form action="/jsp/orderFridayAction">
