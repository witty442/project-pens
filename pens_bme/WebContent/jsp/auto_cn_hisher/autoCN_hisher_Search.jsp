...
function clearForm(path){
	...
	form.action = path + "/jsp/autoCNHISHERAction.do?do=clear2";
	form.submit();
	return true;
}
function search(path){
	var form = document.autoCNHISHERForm;
	form.action = path + "/jsp/autoCNHISHERAction.do?do=search2&action=newsearch";
	form.submit();
	return true;
}
function exportToExcel(path){
	var form = document.autoCNHISHERForm;
	form.action = path + "/jsp/autoCNHISHERAction.do?do=exportToExcel&action=newsearch";
	form.submit();
	return true;
}
function gotoPage(path,currPage){
	var form = document.autoCNHISHERForm;
	form.action = path + "/jsp/autoCNHISHERAction.do?do=search2&currPage="+currPage;
    form.submit();
    return true;
}
function openEdit(path,action,jobId,boxNo,grNo,custGroup,storeCode){
	var form = document.autoCNHISHERForm;
	...
	form.action = path + "/jsp/autoCNHISHERAction.do?do=prepare"+param;
	form.submit();
	return true; 
}
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="bottom: 0;height: 100%;" id="maintab">
  	...
  	<tr id="framerow">
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
            ...
            <c:if test="${autoCNHISHERForm.resultsSearch != null}">
                   <% 
					   int totalPage = autoCNHISHERForm.getTotalPage();
					   int totalRecord = autoCNHISHERForm.getTotalRecord();
					   int currPage =  autoCNHISHERForm.getCurrPage();
					   int startRec = autoCNHISHERForm.getStartRec();
					   int endRec = autoCNHISHERForm.getEndRec();
					%>
						...
						<table id="tblProduct" align="center" border="0" cellpadding="3" cellspacing="2" class="tableSearchNoWidth" width="100%">
							...
							for(int n=0;n<resultList.size();n++){
								...
								<tr class="<%=tabclass%>"> 
								    ...
								    <td class="td_text_center" width="6%"><%=mc.getGrNo()%></td><!-- 5 -->
									<td class="td_text_center" width="6%"><%=mc.getBoxNo()%></td><!-- 6 -->
