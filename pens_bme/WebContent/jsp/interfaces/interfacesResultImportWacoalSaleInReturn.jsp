...
if(interfacesForm.getMonitorItemList() != null){
	 List<MonitorItemBean> monitorItemList = interfacesForm.getMonitorItemList();

     List<MonitorItemResultBean> salesInList = null;
     List<MonitorItemResultBean> returnList = null;
     
     for(int m=0;m<monitorItemList.size();m++){
    	 MonitorItemBean mo = monitorItemList.get(m);
    	 if(mo.getSalesInList() != null && mo.getSalesInList().size()>0){
    		 salesInList = mo.getSalesInList();
    	 }
    	 if(mo.getReturnList() != null && mo.getReturnList().size() >0){
    		 returnList = mo.getReturnList();
    	 }
     }//end for
     if(salesInList != null){
%>
	...
	<%
	 String[] lineArr = null;
	 for(int i=0;i<salesInList.size();i++){
		 MonitorItemResultBean item = salesInList.get(i);
...
<%} %>
 ...
 <p></p>
	<table align="center" border="0" cellpadding="3" cellspacing="1" class="result">
	<tr>
	  <th colspan="8">Return</th>
	</tr>
	<tr>
		<th width="5%">Line in Excel</th>
		<th width="5%">Branch Id</th>
		<th width="25%">Branch name</th>
		<th width="8%">CREDIT_DOC13_SDH</th>
		<th width="8%">DOCUMENT_DATE_SDH</th>
		<th width="8%">Item</th>
		<th width="5%">Qty</th>
		<th width="10%">File Name</th>
	</tr>
	...
	//chkstk_200170125.xls|399|6000P011|บริษัท เอก-ชัย ดีสทริบิวชั่น ซิสเทม จำกัด(สาขาที่ 01747)(THE WALK นครสวรรค์)|16/01/2017|WU1982|121.50|23.00
	 ...
	 for(int i=0;i<returnList.size();i++){
		 MonitorItemResultBean item = returnList.get(i);
		 lineArr = item.getMsg().split("\\|");
	%>
		<c:choose>
			<c:when test="${rows.index %2 == 0}">
