...
<Script>
    function loadme(){
    	/** Case onload from popup Page summited Batch**/
      	 ...
      	 <%if( "searchTaskFinishFromPopupPageNoWait".equals(request.getAttribute("action"))){%>
	         window.opener.searchBatch('<%=request.getContextPath()%>');
	         window.close();
	     <%}%>
	     
      	 /** Case from popup page No wait no Check Status **/
	...
	function submitBatchFromPageByPopupNoWait(path) {
		//alert("path:"+path+",runBatchFromPopup");
		document.batchTaskForm.action = path + "/jsp/batchTaskAction.do?do=runBatchFromPageByPopupNoWait&action=submited";
	...
	<% if( "submited".equals(request.getAttribute("action"))){%>
	   ...
	   function updateProgress(status){
	    ...
	    function checkStatus(){
        	//Case Submit from BatchTask No wait no check status  
        	<%if( "no_wait".equalsIgnoreCase(Utils.isNull(request.getAttribute("no_wait")))){%>
        	   //search display
...
<body topmargin="0" rightmargin="0" leftmargin="0" bottommargin="0"  style="height: 100%;"  style="height: 100%;" onload="loadme()">
          ...
          <!-- Generate BUTTON By Task Config-->
			<table align="center" border="0" cellpadding="3" cellspacing="0" class="body" width="100%">
			    <tr>
					<td align="center" width ="100%">Run BatchTask from popup page No Wait</td>
				</tr>
						...
						<td align="left" width ="100%">
						  <div style="height:80px;align:center">
						     ...
						     <font size="2" color="green">
						                  กดปุ่ม ตรวจสอบสถานะล่าสุด เพื่อทำการตรวจสอบ ว่าสถานะการทำงานสิ้นสุดหรือยัง  (กรณี Report กดดาวน์โหลดไฟล์ได้ หัวข้อ รายละเอียด)
						     </font>
						  </div>
