...
<jsp:useBean id="payInterWWForm" class="com.isecinc.pens.web.pay.PayInterWWForm" scope="session" />
<%
...
function clearForm(path){
	var form = document.payInterWWForm;
	form.action = path + "/jsp/payInterWWAction.do?do=clear2";
	form.submit();
	return true;
}
...
function search(path){
	...
	form.action = path + "/jsp/payInterWWAction.do?do=search2&action=newsearch";
	form.submit();
	return true;
}
function gotoPage(currPage){
	...
	form.action = path + "/jsp/payInterWWAction.do?do=search2&currPage="+currPage;
    form.submit();
    return true;
}
function newDoc(path){
	 var form = document.payInterWWForm;
	var param ="";
	form.action = path + "/jsp/payInterWWAction.do?do=prepare&mode=add"+param;
	form.submit();
	return true; 
}
...
function openEdit(path,docNo){
	...
	form.action = path + "/jsp/payInterWWAction.do?do=prepare&mode=edit"+param;
	form.submit();
	return true; 
}
...
function openCopy(path,docNo){
	 var form = document.payInterWWForm;
	var param ="&docNo="+docNo;
	form.action = path + "/jsp/payInterWWAction.do?do=prepare&mode=copy"+param;
	form.submit();
	return true; 
}

function printReport(path,docNo){
   var url = path+"/jsp/popup/printPayInterWWPopup.jsp?report_name=PayInReport&docNo="+docNo;
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
	    	...
	    	<!-- PROGRAM HEADER -->
	      	...
	      	<jsp:include page="../program.jsp">
				<jsp:param name="function" value="payInterWW"/>
			</jsp:include>
	      	<!-- TABLE BODY -->
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		<tr style="height: 9px;">
	      		...
	      		<tr>
		            ...
		            <td bgcolor="#f8f8f8">
						...
						<!-- BODY -->
						<html:form action="/jsp/payInterWWAction">
            ...
            <c:if test="${payInterWWForm.resultsSearch != null}">
                  	<% 
					   int totalPage = payInterWWForm.getTotalPage();
					   int totalRecord = payInterWWForm.getTotalRecord();
					   int currPage =  payInterWWForm.getCurrPage();
					   int startRec = payInterWWForm.getStartRec();
					   int endRec = payInterWWForm.getEndRec();
					   int pageSize = payInterWWForm.getPageSize();
					...
					<%=PageingGenerate.genPageing(totalPage, totalRecord, currPage, startRec, endRec, no) %>
						...
						<table id="tblProduct" align="center" border="0" cellpadding="3" cellspacing="2" class="tableSearch">
							...
							List<PayBean> resultList = payInterWWForm.getResultsSearch();
