...
function clearForm(path){
	...
	form.action = path + "/jsp/payInterHAction.do?do=clear2";
	form.submit();
	return true;
}
...
function search(path){
	...
	form.action = path + "/jsp/payInterHAction.do?do=search2&action=newsearch";
	form.submit();
	return true;
}
function gotoPage(currPage){
	...
	form.action = path + "/jsp/payInterHAction.do?do=search2&currPage="+currPage;
    form.submit();
    return true;
}
function newDoc(path){
	 var form = document.payInterHForm;
	var param ="";
	form.action = path + "/jsp/payInterHAction.do?do=prepare&mode=add"+param;
	form.submit();
	return true; 
}
...
function openEdit(path,docNo){
	...
	form.action = path + "/jsp/payInterHAction.do?do=prepare&mode=edit"+param;
	form.submit();
	return true; 
}
...
function openCopy(path,docNo){
	 var form = document.payInterHForm;
	var param ="&docNo="+docNo;
	form.action = path + "/jsp/payInterHAction.do?do=prepare&mode=copy"+param;
	form.submit();
	return true; 
}
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
	    	...
	    	<!-- PROGRAM HEADER -->
	      	...
	      	<jsp:include page="../program.jsp">
				<jsp:param name="function" value="payInterH"/>
			</jsp:include>
	      	<!-- TABLE BODY -->
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		<tr style="height: 9px;">
            ...
            <c:if test="${payInterHForm.resultsSearch != null}">
                  	<% 
					   int totalPage = payInterHForm.getTotalPage();
					   int totalRecord = payInterHForm.getTotalRecord();
					   int currPage =  payInterHForm.getCurrPage();
					   int startRec = payInterHForm.getStartRec();
					   int endRec = payInterHForm.getEndRec();
					   int pageSize = payInterHForm.getPageSize();
					...
					<%=PageingGenerate.genPageing(totalPage, totalRecord, currPage, startRec, endRec, no) %>
						...
						<table id="tblProduct" align="center" border="0" cellpadding="3" cellspacing="2" class="tableSearch">
							...
							List<PayBean> resultList = payInterHForm.getResultsSearch();
