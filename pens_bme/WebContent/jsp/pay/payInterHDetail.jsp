<%@page import="com.isecinc.pens.web.pay.PayInterHForm"%>
...
<jsp:useBean id="payInterHForm" class="com.isecinc.pens.web.pay.PayInterHForm" scope="session" />
...
function loadMe(path){
	...
	document.getElementsByName('bean.deptId')[0].value = '${payInterHForm.bean.deptId}';
	
	loadSection();
	<%if( !"".equals(payInterHForm.getBean().getSectionId())){ %>
	  document.getElementsByName('bean.sectionId')[0].value = <%=payInterHForm.getBean().getSectionId()%>;
	<% } %>
...
function printReportPopup(path){
   ...
   // window.open(path + "/jsp/saleOrderAction.do?do=printListOrderProductReport&customerId="+customerId[0].value, "Print2", "width=100,height=100,location=No,resizable=No");
	...
	var url = path+"/jsp/popup/printPayInterHPopup.jsp?report_name=PayInReport&docNo="+docNo;
	PopupCenter(url,'Printer',800,350);
}

function clearForm(path){
	var form = document.payInterHForm;
	form.action = path + "/jsp/payInterHAction.do?do=clear";
	form.submit();
	return true;
}
...
function back(path){
	var form = document.payInterHForm;
	form.action = path + "/jsp/payInterHAction.do?do=prepare2&action=back";
	form.submit();
	return true;
}
...
function save(path){
	var form = document.payInterHForm;
	form.action = path + "/jsp/payInterHAction.do?do=save&action=newsearch";
	form.submit();
	return true;
}
...
function switchFlag(obj,name){
	if(name =='cashFlag'){
		if(obj.checked){
			...
			document.getElementsByName('bean.trFlag')[0].checked = false;
		}
	}else if(name =='chequeFlag'){
		if(obj.checked){
	      	...
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		...
	      		<tr>
		            ...
		            <td bgcolor="#f8f8f8">
						...
						<!-- BODY -->
						<html:form action="/jsp/payInterHAction">
						<jsp:include page="../error.jsp"/>
								...
								<tr>
									...
									<td>		
										<%--  <html:radio property="bean.paymethod" styleId="paymethod" value="C"/>เงินสด
									     ...
									     <html:checkbox property="bean.trFlag" styleId="trFlag" onclick="switchFlag(this,'trFlag');">&nbsp;&nbsp;เงินโอน</html:checkbox>&nbsp;
									</td>
								</tr>
						   </table>
						   ...
						   <table id="tblProduct" align="center" width="75%" border="0" cellpadding="3" cellspacing="2" class="tableSearchNoWidth">
							...
							PayInterHForm pay = (PayInterHForm) session.getAttribute("payInterHForm");
						 ...
						 <%--  <table id="tblProduct" align="center" width="75%" border="0" cellpadding="3" cellspacing="2" class="tableSearchNoWidth">
						   ...
						   <table  border="0" cellpadding="3" cellspacing="0" >
								<tr>
									<td align="left">
									 <c:if test="${payInterHForm.bean.canPrint == true}"> 
