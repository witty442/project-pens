...
function clearForm(path){
	...
	form.action = path + "/jsp/payWhiteAction.do?do=clear2";
	form.submit();
	return true;
}
function search(path){
	...
	form.action = path + "/jsp/payWhiteAction.do?do=search2&action=newsearch";
	form.submit();
	return true;
}
...
function newDoc(path){
	 var form = document.payWhiteForm;
	var param ="";
	form.action = path + "/jsp/payWhiteAction.do?do=prepare&mode=add"+param;
	form.submit();
	return true; 
}
function openEdit(path,docNo){
	...
	form.action = path + "/jsp/payWhiteAction.do?do=prepare&mode=edit"+param;
	form.submit();
	return true; 
}
function openCopy(path,docNo){
	 var form = document.payWhiteForm;
	var param ="&docNo="+docNo;
	form.action = path + "/jsp/payWhiteAction.do?do=prepare&mode=copy"+param;
	form.submit();
	return true; 
}
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="bottom: 0;height: 100%;" id="maintab">
  	...
  	<tr id="framerow">
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
            ...
            <c:if test="${payWhiteForm.resultsSearch != null}">
                  	 <% 
					   int totalPage = payWhiteForm.getTotalPage();
					   int totalRecord = payWhiteForm.getTotalRecord();
					...
					<%=PageingGenerate.genPageing(totalPage, totalRecord, currPage, startRec, endRec, no) %>
						...
						<table id="tblProduct" align="center" border="0" cellpadding="3" cellspacing="2" class="tableSearch">
							...
							List<PayBean> resultList = payWhiteForm.getResultsSearch();
