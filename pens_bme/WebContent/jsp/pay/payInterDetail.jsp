...
<%@page import="com.isecinc.pens.web.pay.PayInterForm"%>
...
function loadMe(path){
	...
	document.getElementsByName('bean.deptId')[0].value = '${payInterForm.bean.deptId}';
	
	loadSection();
	<%if( !"".equals(payInterForm.getBean().getSectionId())){ %>
	  document.getElementsByName('bean.sectionId')[0].value = <%=payInterForm.getBean().getSectionId()%>;
	<% } %>
...
function clearForm(path){
	...
	form.action = path + "/jsp/payInterAction.do?do=clear";
	form.submit();
	return true;
}
...
function back(path){
	var form = document.payInterForm;
	form.action = path + "/jsp/payInterAction.do?do=prepare2&action=back";
	form.submit();
	return true;
}
...
function save(path){
	var form = document.payInterForm;
	form.action = path + "/jsp/payInterAction.do?do=save&action=newsearch";
	form.submit();
	return true;
}
		            ...
		            <td bgcolor="#f8f8f8">
						...
						<jsp:include page="../error.jsp"/>
						   ...
						   <table id="tblProduct" align="center" width="75%" border="0" cellpadding="3" cellspacing="2" class="tableSearchNoWidth">
							...
							PayInterForm pay = (PayInterForm) session.getAttribute("payInterForm");
						 ...
						 <%--  <table id="tblProduct" align="center" width="75%" border="0" cellpadding="3" cellspacing="2" class="tableSearchNoWidth">
						   ...
						   <table  border="0" cellpadding="3" cellspacing="0" >
								<tr>
									<td align="left">
									 <c:if test="${payInterForm.bean.canPrint == true}"> 
