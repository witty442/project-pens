<%@page import="com.isecinc.pens.web.autosubin.AutoSubInBean"%>
...
<jsp:useBean id="autoSubInForm" class="com.isecinc.pens.web.autosubin.AutoSubInForm" scope="session" /> 
<%
...
function saveAction(path){
	var form = document.autoSubInForm;
	if(confirm("ยืนยันบันทึกข้อมูล")){
	  form.action = path + "/jsp/autoSubInAction.do?do=save";
	  form.submit();
	  return true;
	}
	return false;
...
function cancelAction(path){
	...
	if(confirm("ยืนยันยกเลิกรายการ")){
	  form.action = path + "/jsp/autoSubInAction.do?do=cancelAction";
	  form.submit();
	  return true;
	}
	return false;
...
function backAction(path){
	var form = document.autoSubInForm;
	form.action = path + "/jsp/autoSubInAction.do?do=search2&action=back";
	form.submit();
	return true;
}
...
function sumTotal(){
 ...
 function getProductKeypress(e,pensItemObj,rowId){
	...
	function getProductModel(pensItemObj,rowId){
		...
		var getData = $.ajax({
				url: "${pageContext.request.contextPath}/jsp/autoCN/getProductAjax.jsp",
    	...
    	<td background="${pageContext.request.contextPath}/images2/content01.png" valign="top">
	    	...
	    	<!-- PROGRAM HEADER -->
	      	...
	      	<jsp:include page="../program.jsp">
				<jsp:param name="function" value="AutoSubIn"/>
			</jsp:include>
	      	<!-- TABLE BODY -->
	      	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="txt1">
	      		<tr style="height: 9px;">
	      		...
	      		<tr>
		            ...
		            <td bgcolor="#f8f8f8">
						...
						<!-- BODY -->
						<html:form action="/jsp/autoSubInAction">
						    ...
						    <table align="center" border="0" cellpadding="3" cellspacing="0" >
								...
								<tr>
									...
									<td colspan="2">		
								       ...
								       &nbsp;&nbsp;
								       Sub Inventory 
								       <html:text property="bean.subInv" styleId="subInv" size="10" styleClass="disableText"  readonly="true"/>
									</td>
								</tr>
								<tr>
                                    <td align="right">ขนส่งโดย </td>
									<td colspan="2">		
										<html:text property="bean.forwarder" styleId="forwarder" size="15" readonly="true" styleClass="disableText"/>
								       &nbsp;&nbsp;
								       จำนวนกล่องที่รับจากขนส่ง 
								       <html:text property="bean.forwarderBox" styleId="forwarderBox" size="10" styleClass="disableText"  readonly="true"/>
									</td>
								</tr>
						   </table>
                    <!-- table data -->
           
                      <%if(autoSubInForm.getBean().isCanSave()) { %>
                       ...
                       <%} %>
						<table id="tblProduct" align="center" border="0" cellpadding="3" cellspacing="2" class="tableSearchNoWidth" width="100%">
							...
							List<AutoSubInBean> resultList = autoSubInForm.getResultsSearch();
							String keyData = "";
							...
							for(int n=0;n<resultList.size();n++){
								AutoSubInBean mc = (AutoSubInBean)resultList.get(n);
								if(n%2==0){ 
									tabclass="lineO";
								}
	            ...
	            <table id="tblProductBtn" align="center" border="0" cellpadding="3" cellspacing="2"  width="65%">
                         <tr><td align="center">
                            <%if(autoSubInForm.getBean().isCanSave()) { %>
                                <input type="button" name="add" value="Approve to Transfer" onclick="saveAction('${pageContext.request.contextPath}')" class="newPosBtnLong"/>
                            <%} %>
                            &nbsp;&nbsp;
                            <input type="button" name="delete" value="  ปิดหน้าจอ  " onclick="backAction('${pageContext.request.contextPath}')" class="newPosBtnLong"/>
                           
                            <%if(autoSubInForm.getBean().isCanCancel()) { %>
                              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
