...
function cancel(path){
	if(confirm("ยืนยันการยกเลิกรายการนี้")){
		var form = document.pickStockForm;
		form.action = path + "/jsp/pickStockAction.do?do=cancelAllPartBox";
		form.submit();
		return true;
	}
	return false;
...
function confirmAction(path){
	...
	if(confirm("ยันยันการ Confirm ข้อมูล")){
		 form.action = path + "/jsp/pickStockAction.do?do=confirmAllPartBox";
		 form.submit();
		 return true;
	}
	return false;
...
function save(path){
	...
	if(confirm("ยันยันการบันทึกข้อมูล")){
	   form.action = path + "/jsp/pickStockAction.do?do=savePartBox";
	   form.submit();
	   return true;
	}
	return false;
		            ...
		            <td bgcolor="#f8f8f8">
						...
						<jsp:include page="../error.jsp"/>
						    ...
						    <table align="center" border="0" cellpadding="3" cellspacing="0" >
						       <tr>
                                  <td colspan="3" align="center"><font size="3"><b>เบิกทั้งกล่อง เฉพาะกล่องที่มีการเบิกบางส่วน</b></font></td>
							   </tr>
						       <tr>
									...
									<c:forEach var="results" items="${pickStockForm.results}" varStatus="rows">
										<c:choose>
											...
											<tr class="<c:out value='${tabclass}'/>">
												...
												<td class="data2_total_box_qty" align="center">${results.totalBoxQty}
												   <input type="hidden" name="totalqty" value ="${results.totalBoxQty}" size="20" readonly class="disableText"/>
												</td>
