...
if(session.getAttribute("statusProcessQueryList") == null){
	List<References> billTypeList = new ArrayList();
	References ref = new References("","");
	billTypeList.add(ref);
	ref = new References("ALL","ALL Stock");
	billTypeList.add(ref);
	ref = new References("ON PROCESS","ON Process");
	billTypeList.add(ref);
	session.setAttribute("statusProcessQueryList",billTypeList);
}
...
function openStatusPopup(path){
	if(checkProcess()){
	    var param = "";
		url = path + "/jsp/popup/showStatusPopup.jsp?"+param;
		window.open(encodeURI(url),"",
				   "menubar=no,resizable=no,toolbar=no,scrollbars=yes,width=600px,height=540px,status=no,left="+ 50 + ",top=" + 0);
    }
}

function setStatusMainValue(code,desc){
	//alert(types+":"+desc);
	document.getElementsByName("bean.status")[0].value = code;
	document.getElementsByName("bean.statusDesc")[0].value = desc;
}

function checkProcess(){
	if(document.getElementsByName("bean.process")[0].value !=""){
		alert("ให้เลือกได้เพียงระดับใดระดับหนึ่งเท่านั้น");
		document.getElementsByName("bean.status")[0].value = "";
		return false;
	}
	return true;
}
...
function checkStatus(){
	if(document.getElementsByName("bean.status")[0].value !=""){
		alert("ให้เลือกได้เพียงระดับใดระดับหนึ่งเท่านั้น");
		document.getElementsByName("bean.process")[0].value = "";
		return false;
	}
	return true;
}
						   ...
						   <div align="center">
						    <table align="center" border="0" cellpadding="3" cellspacing="0" >
						        <tr>
                                    <td> Warehouse</td>
									<td colspan="3">					
										 <html:select property="bean.wareHouse">
						       ...
						       <tr>
								...
								<tr>
                                    <td> เรียกดูระดับ process </td>
									<td colspan="3">					
										 <html:select property="bean.process" onchange="checkStatus()">
											<html:options collection="statusProcessQueryList" property="key" labelProperty="name"/>
									    </html:select>
									</td>
								</tr>
								<tr>
                                    <td>เรียกดู ระดับ  สถานะ </td>
									<td colspan="3">
						                <html:text property="bean.status" styleId="status" size="20" readonly="true"/>					    
									     <input type="button" name="x1" value="..." onclick="openStatusPopup('${pageContext.request.contextPath}')"/>
									    <html:text property="bean.statusDesc" styleId="statusDesc" readonly="true" styleClass="disableText" size="50"/>
									</td>
								</tr>	
								<tr>
                                    <td> แสดงตาม  </td>
							...
							<table id="tblProduct" align="center" border="0" cellpadding="3" cellspacing="1" class="tableSearch2">
								  ...
								  <c:forEach var="results" items="${stockQueryForm.results}" varStatus="rows">
							       ...
							       <tr>
										...
										<td class="search2_jobname"><b>${stockQueryForm.bean.totalQty}</b></td>					
								   </tr>
					...
					<c:if test="${stockQueryForm.bean.summaryType =='SummaryByPensItem'}">
							<table id="tblProduct" align="center" border="0" cellpadding="3" cellspacing="1" class="tableSearch3">
								 ...
								 <tr>
										...
										<td class="search3_groupCode"><b>${stockQueryForm.bean.totalQty}</b></td>					
								   </tr>
						</table>
					</c:if>
				</c:if>
