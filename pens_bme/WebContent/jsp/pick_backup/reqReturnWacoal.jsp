...
function clearForm(path){
	...
	form.action = path + "/jsp/reqReturnWacoalAction.do?do=clear";
	form.submit();
	return true;
}
...
function cancel(path){
	if(confirm("ยืนยันการยกเลิกรายการนี้")){
		var form = document.reqReturnWacoalForm;
		form.action = path + "/jsp/reqReturnWacoalAction.do?do=cancel";
		form.submit();
		return true;
	}
	return false;
...
function back(path){
	var form = document.reqReturnWacoalForm;
	form.action = path + "/jsp/reqReturnWacoalAction.do?do=prepare2&action=back";
	form.submit();
	return true;
}
...
function save(path){
	...
	if(confirm("ยันยันการบันทึกข้อมูล")){
	   form.action = path + "/jsp/reqReturnWacoalAction.do?do=save";
	   form.submit();
	   return true;
	}
	return false;
